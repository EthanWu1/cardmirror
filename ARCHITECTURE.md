...
**Persistent shared `.cmir` files.** `POST /relay/docs` creates a durable
document room using the same encrypted update/snapshot/stream contract as
temporary rooms, but the sweeper skips it until it is archived/tombstoned. The
native `.cmir` envelope can embed `{docId, roomId, shareCode, createdAt}`; open
paths auto-connect that file into the existing editor view rather than spawning
a separate session document. If a resumable local CRDT record exists, reconnect
uses that record so offline edits survive. If no record exists, the room state
is authoritative and CardMirror asks before replacing local file contents; when
the relay is unreachable it leaves the file open as a local copy and exposes the
embedded share code for retry. The security boundary is the share code in the
file: there are not yet roles, revocation, read-only links, or per-user audit.
