...
Use **FLOW** on Home or **Create Flow** in the ribbon to open the native
Flow workspace. CardMirror saves these files as `.cmflow`.

Native Flow can import or export Verba Flowline `.flowline.json` files.
The legacy Excel Flow bridge stays under its legacy labels, so existing
Verbatim Flow workflows remain separate from native Flow.

In three-pane mode, CardMirror asks where to route a Flow: **Slot 1**,
**Slot 2**, **Slot 3**, or **Open Separate**. **Open Separate** uses
desktop host support when the desktop app can create another window.
...
**What is shared.** A live session starts from one open document and gives
everyone an encrypted, real-time view of the same CRDT document. When that
document is saved as `.cmir`, CardMirror can also store a shared-document handle
inside the file. Opening that shared `.cmir` later reconnects to the same room,
so it behaves more like a durable shared document than the early "one-time call"
model. The readable document still lives on your devices; the relay only stores
encrypted updates and snapshots it cannot read.

**Shared files are edit-capable handles.** If a shared `.cmir` is in Dropbox,
Google Drive, a team folder, or an email thread, anyone who can read that file
has the room id and share code needed to reconnect and edit, assuming they can
reach the same relay. There are not yet per-user roles, read-only links, or
revocation controls, so treat a shared `.cmir` like an edit link.
...
If you open a shared `.cmir` while the relay is unreachable and CardMirror does
not have a saved session record for that room, it leaves the file open as a
local copy. The collaboration chip says **Shared doc: local copy**, and **Copy
Session Share Code** still copies the embedded code. Use **Join Collaboration
Session** or **Start Collaboration Session** while that file is focused to retry
the reconnect. When reconnecting without a saved session record, CardMirror asks
before replacing the local file contents with the room version.
