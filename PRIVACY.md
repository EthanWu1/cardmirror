...
- If you use co‑editing: local records of your collaboration sessions, plus any
  shared-document metadata embedded in `.cmir` files you save.
...
- **How it's protected.** Everything exchanged in a session or persistent shared
  document — document edits, comments, and cursor presence — is
  **end‑to‑end encrypted** with a key that exists only in the invite/share link,
  embedded shared `.cmir` metadata, and participants' devices. The relay stores
  and forwards encrypted data it cannot read.
- **Shared `.cmir` files.** When you save a shared document as `.cmir`, the file
  can carry the room id and encrypted share code needed to reconnect later. If
  you put that file in Dropbox, Google Drive, a team folder, or email it to
  someone, anyone who can read the file can use that share code to join/edit the
  shared document, provided they can reach the relay. Treat shared `.cmir` files
  like edit-capable links.
...
- **Retention:**
  - When the host chooses **"End session,"** the encrypted session or persistent
    shared-document data is **deleted from the relay immediately**.
  - Otherwise, an inactive session's encrypted data is **automatically deleted
    after 7 days of inactivity**.
  - Persistent shared-document rooms are kept until the document is ended or
    archived, so a saved shared `.cmir` can reconnect later.
...
The current collaboration security model is link/key based, not account based:
there are no per-user roles, read-only participants, revocation lists, or audit
logs in the shared-document room itself. For persistent shared `.cmir` files,
protecting access to the file and its share code is what controls who can edit.
...
- **A collaboration session or shared document:** the host's **"End session"**
  deletes the encrypted room data from the relay immediately. Temporary sessions
  otherwise expire after 7 days of inactivity; persistent shared documents stay
  on the relay until ended/archived. You can also clear a session's local record
  on your device.
