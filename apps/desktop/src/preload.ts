...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  chooseSaveLocation: (
    ...
    ipcRenderer.invoke('host:choose-save-location', suggestedName, opts) as Promise<{
      name: string;
      handle: string;
    } | null>,

  saveExisting: (handle: string, bytes: Uint8Array, opts?: SaveExistingOptions) =>
    ipcRenderer.invoke('host:save-existing', handle, bytes, opts) as Promise<SaveExistingResult>,
  ...
  listFilesRecursive: (dir: string, ext: string) =>
    ...
    >,
  /** Cached + persisted recursive openable editor-file listing for the
   *  command-palette file search (returns instantly from main's cache,
  ...
  /** Native frameless-window controls. `close` intentionally goes through the
   *  normal close event in main so renderer unsaved-change prompts still run. */
  windowCommand: (action: 'minimize' | 'maximize-or-restore' | 'close') =>
    ipcRenderer.invoke('host:window-command', action),
