 ...
 *
 * The relay contract here is identical to the self-hosted `/relay` API. A
 * packaged build has no relay default unless CI injects one at build time.
 */
 ...
 *  overrides can point dev at a local mock, e.g.:
 *    PAIRING_RELAY_URL=http://127.0.0.1:3200 PAIRING_TOKEN=<token> \
 *      npm run desktop:dev
 *
 *  The URL is not secret and can be injected at build/run time via
 *  PAIRING_RELAY_URL. The TOKEN is NOT hard-coded here: this is a PUBLIC repo,
 *  so the real relay token is injected at build/run
...
export function registerPairingIpc(): void {
  ...
  ipcMain.handle('host:collab-relay-defaults', (): { url: string; token: string } => {
    return { url: relayUrl(), token: sharedRelayToken() };
  });
