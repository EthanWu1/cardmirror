...
type EditorFileFormat = 'cmir' | 'docx' | 'cmflow';

function formatForPath(filePath: string): EditorFileFormat | null {
...
function pickFileFromArgv(argv: readonly string[]): string | null {
  for (const a of argv) {
    ...
    if (isOpenableEditorPath(a)) return a;
  }
  return null;
}
...
async function scanCmirFiles(root: string): Promise<CmirFileEntry[]> {
  ...
  const isOpenable = (name: string): boolean => {
    ...
    return isOpenableEditorPath(name);
  };
  async function walk(cur: string): Promise<void> {
    let entries;
    try {
...
async function defaultSaveDialogPath(
  suggestedName: string,
  opts: { nearPath?: string } | undefined,
): Promise<string> {
...
ipcMain.handle(
  ...
  async (
    ...
    const defaultPath = await defaultSaveDialogPath(suggestedName, opts);
...
ipcMain.handle(
  ...
  async (
    ...
    handle: string,
    bytes: unknown,
    opts?: { force?: boolean; conflictCopiesForDocx?: boolean },
  ) => {
    ...
    return await saveExistingDoc(handle, bytesToBuffer(bytes), {
      force: opts?.force === true,
      conflictCopiesForDocx: opts?.conflictCopiesForDocx === true,
    });
  },
);
  ...
  for (const name of entries) {
    ...
    try {
      ...
      results.push({
        ...
        format:
          parsed.format === 'cmir' || parsed.format === 'docx' || parsed.format === 'cmflow'
            ? parsed.format
            : null,
...
ipcMain.handle('host:window-command', async (event, action: unknown) => {
  const win = BrowserWindow.fromWebContents(event.sender);
...
const TERMS_URL = 'https://github.com/EthanWu1/cardmirror/blob/main/TERMS.md';
