...
…and persistent DOCUMENT rooms, which reuse the same room update/snapshot/
stream contract but are not idle-GC'd while active:

  POST   /relay/docs                        create → {docId, roomId}
...
from sqlalchemy import BigInteger, Boolean, Column, DateTime, Index, Integer, JSON, String, create_engine, event
...
SequenceId = BigInteger().with_variant(Integer(), "sqlite")
...
class RelayMessage(Base):
    ...
    body = Column(JsonPayload, nullable=False)
...
class RelayRoomUpdate(Base):
    ...
    id = Column(SequenceId, primary_key=True, autoincrement=True)
...
class RelayRoomSnapshot(Base):
    ...
    blob = Column(String, nullable=False)  # base64 ciphertext, opaque
    covers_through_seq = Column(SequenceId, nullable=False)
    created_at = Column(DateTime, default=datetime.utcnow, nullable=False)
...
class RelayDocument(Base):
    ...
    # Opaque server-side id. In the first persistent-doc slice this equals
    # room_id, so the existing rooms transport can be reused unchanged.
    id = Column(String, primary_key=True)
...
def _sweep(db: Session) -> int:
    ...
    for room in idle:
        ...
        if room.tombstoned:
            db.query(RelayDocument).filter(RelayDocument.room_id == room.id).delete(
                synchronize_session=False
            )
            db.delete(room)
        else:
...
def _tombstone_room(db: Session, room: RelayRoom) -> bool:
    if room.tombstoned:
        return False
    doc = db.query(RelayDocument).filter(RelayDocument.room_id == room.id).first()
    if doc is not None:
        doc.archived = True
    room.tombstoned = True
    room.bytes_used = 0
...
def delete_document_room(doc_id: str, db: Session = Depends(get_db)) -> Response:
    ...
    if room is None:
        ...
        pushed = _tombstone_room(db, room)
    db.commit()
