 ...
 * partner's store, lease ads render/clear/remap as 0x02 frames, and
 * room membership heartbeats keep avatars visible even when the peer
 * has no active cursor decoration.
 */
...
describe('M4 presence cursors', () => {
  it('keeps a partner in the roster from membership presence even without a cursor frame', async () => {
    const { session: host } = await CollabSession.host({
      pmDoc: simpleDoc('presence member doc'),
      client,
      flushMs: 40,
    });
    const cursors = installCursorPresence(host, () => null);
    const remotePeer = 'remote-peer-without-cursor';
    ...
    cursors.applyRemote(frame);

    const roster = cursors.presence();
    expect(roster.find((p) => p.peer === remotePeer)).toMatchObject({
      name: 'Priya Shah',
      color: '#2266cc',
      self: false,
    });
    expect(cursors.visiblePeers()).toContain(remotePeer);

    cursors.dispose();
    await host.stop();
  });

  it('jumps to the partner cursor advertised by membership presence', async () => {
    const { session: host } = await CollabSession.host({
      pmDoc: simpleDoc('jump target is inside this sentence'),
      client,
      flushMs: 40,
    });
    ...
    const view = new EditorView(mount, {
      state: EditorState.create({
        schema,
        doc: simpleDoc('jump target is inside this sentence'),
      }),
    });
    const cursors = installCursorPresence(host, () => view);
    const remotePeer = 'remote-peer-with-cursor';
    const payload = new TextEncoder().encode(
      JSON.stringify({
        peer: remotePeer,
        name: 'Priya Shah',
        color: '#2266cc',
        cursor: { from: 8, to: 14, anchor: 8, head: 14 },
        at: Date.now(),
      }),
    );
    const frame = new Uint8Array(payload.length + 1);
    ...
    expect(cursors.jumpToPeer(remotePeer)).toBe(true);
    expect(view.state.selection.from).toBe(8);
    expect(view.state.selection.to).toBe(14);
    expect(cursors.jumpToPeer('unknown-peer')).toBe(false);
