...
describe('collab session end-to-end', () => {
  it('uses a low-latency default flush for realtime typing', async () => {
    ...
    typeAfter(hostView, 'default', ' live');
    await sleep(300);
    expect(docText(joinView.state.doc)).toContain('default live sync');
  ...
  it('hosts a persistent document room and still seeds normal joiners', async () => {
    const seed = simpleDoc('persistent seed');
    const { session: host, shareCode, sharedDoc } = await CollabSession.hostPersistent({
      pmDoc: seed,
      client,
      ...FAST,
    });
    expect(sharedDoc.docId).toBe(host.roomId);
    expect(sharedDoc.roomId).toBe(host.roomId);
    expect(sharedDoc.shareCode).toBe(shareCode);
    expect(Number.isNaN(Date.parse(sharedDoc.createdAt))).toBe(false);

    const decoded = decodeShareCode(shareCode)!;
    expect(decoded.roomId).toBe(sharedDoc.roomId);
    const joiner = await CollabSession.join({ ...decoded, client, ...FAST });
    const joinView = mkView(joiner.plugins());
    await settle();
  ...
  it('keeps a local caret anchored to the same text when a partner pastes above it', async () => {
    const { host, hostView, joiner, joinView } = await hostAndJoin(
      simpleDoc('alpha target omega'),
    );
    const target = findText(joinView.state.doc, 'target');
    joinView.dispatch(
      joinView.state.tr.setSelection(TextSelection.create(joinView.state.doc, target.from + 3)),
    );

    hostView.dispatch(hostView.state.tr.insertText('REMOTE PASTE ', 1));
    await sleep(250);

    const movedTarget = findText(joinView.state.doc, 'target');
    expect(joinView.state.selection.from).toBe(movedTarget.from + 3);
    expect(joinView.state.selection.to).toBe(movedTarget.from + 3);
    expect(joinView.state.doc.eq(hostView.state.doc)).toBe(true);
