...
describe('collab UI flows through the editor seams', () => {
  it('prompts before a no-record shared .cmir replaces local content', async () => {
    ...
    const cancelled = collabUi.connectSharedDocFlow(deps, sharedDoc);
    await settle();
    await sleep(100);
    clickPromptButton('Keep Local Copy');
    expect(await cancelled).toBe(false);
    await settle();
    expect(collabUi.activeSession()).toBeNull();
    expect(docText(view.state.doc)).toContain('offline draft not synced');
    expect(storedSharedDoc).toBeNull();

    const accepted = collabUi.connectSharedDocFlow(deps, sharedDoc);
    await settle();
    await sleep(100);
    clickPromptButton('Connect Shared Document');
