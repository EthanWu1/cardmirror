...
describe('native format (.cmir)', () => {
  ...
  it('round-trips persistent collaboration metadata', () => {
    const original = makeSampleDoc();
    const sharedDoc = {
      docId: 'shared-doc-1',
      roomId: '0123456789abcdef0123456789abcdef',
      shareCode: 'cmshare1.0123456789abcdef0123456789abcdef.AAECAwQFBgcICQoLDA0ODw',
      createdAt: '2026-07-13T00:00:00.000Z',
    };
    const bytes = serializeNative(original, { sharedDoc });
    const parsed = parseNative(bytes);
    expect(parsed.doc.eq(original)).toBe(true);
    expect(parsed.sharedDoc).toEqual(sharedDoc);
  });
