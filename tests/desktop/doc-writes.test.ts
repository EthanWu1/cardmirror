...
describe('saveExistingDoc — changed-on-disk guard', () => {
  ...
  it('saves a docx conflicted copy when the original changed on disk', async () => {
    const p = await openedDoc('original docx', 'Aff.docx');
    await fs.writeFile(p, 'edited somewhere else');

    const result = await saveExistingDoc(p, Buffer.from('my edits'), {
      conflictCopiesForDocx: true,
    });

    expect(result).toMatchObject({ status: 'conflict', reason: 'changed' });
    if (result.status !== 'conflict') throw new Error('Expected docx save to create a conflict copy.');
    expect(await read(p)).toBe('edited somewhere else');
    expect(result.conflictPath).not.toBe(p);
    expect(result.conflictName).toMatch(/^Aff \(CardMirror conflicted copy .+\)\.docx$/);
    expect(await read(result.conflictPath)).toBe('my edits');
  });
});
...
describe('atomic writes', () => {
  ...
  it('falls back to copying over the existing file when rename is denied', async () => {
    const p = await openedDoc('old');
    const realRename = fs.rename.bind(fs);
    let deniedReplace = false;
    const renameSpy = vi.spyOn(fs, 'rename').mockImplementation(async (from, to) => {
      if (String(from).endsWith('.cmtmp') && String(to) === p) {
        deniedReplace = true;
    ...
    try {
      await saveExistingDoc(p, Buffer.from('new'));
    } finally {
      renameSpy.mockRestore();
    }

    expect(deniedReplace).toBe(true);
    expect(await read(p)).toBe('new');
    const leftovers = (await fs.readdir(caseDir)).filter((f) => f.includes('.cmtmp'));
    expect(leftovers).toEqual([]);
  });

  it('retries when cloud-sync fallback writes are briefly locked', async () => {
    const p = await openedDoc('old');
    const realRename = fs.rename.bind(fs);
    ...
    const copySpy = vi.spyOn(fs, 'copyFile').mockImplementation(async (from, to, mode) => {
      copyAttempts++;
      if (copyAttempts === 1 && String(to) === p) {
        const err = new Error('resource busy or locked') as NodeJS.ErrnoException;
        err.code = 'EBUSY';
        throw err;
      }
      await realCopyFile(from, to, mode as number | undefined);
    });
    const writeSpy = vi.spyOn(fs, 'writeFile').mockImplementation(async (target, data, options) => {
      ...
      await realWriteFile(target, data, options as Parameters<typeof fs.writeFile>[2]);
    });
  ...
  it('directly overwrites as a last resort when copyFile stays locked', async () => {
    const p = await openedDoc('old');
    const realRename = fs.rename.bind(fs);
    const realWriteFile = fs.writeFile.bind(fs);
    let directWrites = 0;
    ...
    const copySpy = vi.spyOn(fs, 'copyFile').mockImplementation(async (_from, to) => {
      if (String(to) === p) {
        const err = new Error('resource busy or locked') as NodeJS.ErrnoException;
        err.code = 'EBUSY';
        throw err;
    ...
    const writeSpy = vi.spyOn(fs, 'writeFile').mockImplementation(async (target, data, options) => {
      if (String(target) === p) directWrites++;
      await realWriteFile(target, data, options as Parameters<typeof fs.writeFile>[2]);
    });
  ...
  it('saves an unchanged docx in place when atomic replace is denied', async () => {
    const p = await openedDoc('old', 'Aff.docx');
    const realRename = fs.rename.bind(fs);
    const realCopyFile = fs.copyFile.bind(fs);
    const realWriteFile = fs.writeFile.bind(fs);
    let copyAttempts = 0;
    ...
    const writeSpy = vi.spyOn(fs, 'writeFile').mockImplementation(async (target, data, options) => {
      if (String(target) === p) {
        throw new Error('direct write should not be needed');
      }
      await realWriteFile(target, data, options as Parameters<typeof fs.writeFile>[2]);
    });

    let result: Awaited<ReturnType<typeof saveExistingDoc>>;
    try {
      result = await saveExistingDoc(p, Buffer.from('new'), {
        conflictCopiesForDocx: true,
      });
    } finally {
      renameSpy.mockRestore();
    ...
    expect(await read(p)).toBe('new');
    const conflictCopies = (await fs.readdir(caseDir)).filter((f) =>
      f.includes('CardMirror conflicted copy'),
    );
