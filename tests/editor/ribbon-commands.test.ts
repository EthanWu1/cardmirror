...
describe('global hotkey fallback (focus outside the editor)', () => {
  ...
  function withNavigatorPlatform(platform: string, fn: () => void): void {
  ...
  it('keeps real Control separate from Command on macOS', () => {
    withNavigatorPlatform('MacIntel', () => {
      expect(ribbonKeyStringFor(kbd({ key: 'm', code: 'KeyM', ctrlKey: true }))).toBe('Ctrl-m');
      expect(ribbonKeyStringFor(kbd({ key: 'm', code: 'KeyM', metaKey: true }))).toBe('Mod-m');
      expect(formatKeyForDisplay('Ctrl-m')).toBe('⌃m');
