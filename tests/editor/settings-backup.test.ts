...
function withFakeLocalStorage(run: (store: Storage) => void): void {
...
describe('defaultZoomPct (the open-at body-zoom default)', () => {
  it('clamps to 50–300% and rounds to the nearest 10', () => {
    ...
    over.replaceAll({ defaultZoomPct: 700 } as never);
    expect(over.get('defaultZoomPct')).toBe(500);
  ...
  it('multiDocConfirmModeSwitch defaults on and only explicit false disables it', () => {
    expect(new SettingsStore().get('multiDocConfirmModeSwitch')).toBe(true);
    const off = new SettingsStore();
    off.replaceAll({ multiDocConfirmModeSwitch: false } as never);
    expect(off.get('multiDocConfirmModeSwitch')).toBe(false);
    const garbage = new SettingsStore();
    garbage.replaceAll({ multiDocConfirmModeSwitch: 'no' } as never);
    expect(garbage.get('multiDocConfirmModeSwitch')).toBe(true);
  });
});

describe('gestureZoom', () => {
  it('defaults pinch / Ctrl-scroll zoom on', () => {
    expect(new SettingsStore().get('gestureZoom')).toBe(true);
  });

  it('fills missing imported gestureZoom with the default-on value', () => {
    const s = new SettingsStore();
    s.replaceAll({ commentAuthor: 'Imported' });
    expect(s.get('gestureZoom')).toBe(true);
  });

  it('migrates the old persisted default-off value once, then respects an explicit off', () => {
    withFakeLocalStorage((store) => {
      store.setItem('pmd-settings', JSON.stringify({ gestureZoom: false }));

      const migrated = new SettingsStore();
      expect(migrated.get('gestureZoom')).toBe(true);
      expect(JSON.parse(store.getItem('pmd-settings') ?? '{}').gestureZoom).toBe(true);

      migrated.set('gestureZoom', false);
      expect(new SettingsStore().get('gestureZoom')).toBe(false);
    });
  });
});
