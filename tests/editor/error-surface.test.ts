...
describe('isFileBusyError', () => {
  it('classifies Electron EBUSY/resource-busy save errors', () => {
    expect(
      isFileBusyError(
        new Error(
          "Error invoking remote method 'host:save-existing': Error: EBUSY: resource busy or locked, open 'C:\\x.docx'",
        ),
      ),
    ).toBe(true);
    expect(isFileBusyError(new Error('EBUSY: resource busy or locked'))).toBe(true);
  });

  it('rejects changed, gone, permission, and non-Errors', () => {
    expect(isFileBusyError(new Error('EMODIFIED: "x.docx" changed on disk'))).toBe(false);
    expect(isFileBusyError(new Error('ENOENT: no such file or directory'))).toBe(false);
    expect(isFileBusyError(new Error('EACCES: permission denied'))).toBe(false);
    expect(isFileBusyError('EBUSY-ish string')).toBe(false);
    expect(isFileBusyError(null)).toBe(false);
  });
});
