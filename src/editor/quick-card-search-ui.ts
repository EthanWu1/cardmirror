...
async function runWarmPass(
  ...
  try {
    ...
    for (const path of pins) {
      ...
      if (fileFormat(path) === 'cmflow') continue;
...
export interface QuickCardSearchOptions {
  ...
  /** Evidence mode searches full text in configured file-search folders. */
  mode?: 'everything' | 'evidence';
  /** Open a file and jump to a text descriptor. Used by evidence results. */
...
function evidenceResult(row: EvidenceSearchRow): PaletteResult {
  return {
    source: 'evidence',
    name: row.label || row.text,
    meta: `${row.fileName} - ${evidenceKindLabel(row.kind)}`,
    matchedName: false,
    snippet: row.snippet,
...
function badgeText(r: PaletteResult): string {
  switch (r.source) {
    ...
    case 'file':
      ...
      return (r.fileFormat ?? fileFormat(r.filePath ?? r.name)).toUpperCase();
    case 'fileobject':
...
function resultKey(r: PaletteResult): string {
  ...
  const anchor = r.evidenceAnchor?.quote ?? '';
  return `${r.source}:${id}:${anchor}`;
}
...
function canSearchInsideFile(r: PaletteResult | undefined): boolean {
  if (!r || r.source !== 'file') return false;
  const format = r.fileFormat ?? fileFormat(r.filePath ?? r.name);
...
class QuickCardSearchUI {
  ...
  open(opts: QuickCardSearchOptions): void {
    ...
    root.className = this.mode === 'evidence' ? 'pmd-qcs pmd-qcs-evidence' : 'pmd-qcs';
    root.innerHTML = `
      ...
      <input class="pmd-qcs-input" type="text" spellcheck="false" ${AUTOFILL_IGNORE_ATTRS}
             placeholder="${this.mode === 'evidence' ? 'Search evidence...' : SEARCH_PLACEHOLDER}" aria-label="${this.mode === 'evidence' ? 'Search evidence' : 'Search'}" />
    ...
    switch (e.key) {
      ...
      case 'Tab':
        ...
        if (this.results[this.selected]?.source === 'file') {
          if (canSearchInsideFile(this.results[this.selected])) void this.enterInFile();
          break;
        }
  ...
  private scheduleSearch(): void {
    ...
    const run = (): void => {
      this.searchRaf = 0;
      this.searchTimer = 0;
      if (this.root) this.runSearch();
    };
    if (this.mode === 'evidence') {
      this.searchTimer = window.setTimeout(run, 80);
    } else if (typeof window.requestAnimationFrame === 'function') {
      this.searchRaf = window.requestAnimationFrame(run);
    } else {
      this.searchTimer = window.setTimeout(run, 0);
    }
  }
  ...
  private flushScheduledSearch(): void {
    if (!this.searchRaf && !this.searchTimer) return;
    this.cancelScheduledSearch();
    if (this.root) this.runSearch();
  }
  ...
  private runSearch(): void {
    ...
    if (prefix === 'q') {
      ...
      const quickCards = sourceCap(searchQuickCardSource(query));
      const dropzone = dropzoneOn() ? sourceCap(searchDropzoneSource(query)) : [];
      const commands = sourceCap(searchCommandSource(query));
      const settingToggles = sourceCap(searchSettingToggleSource(query));
      const settingCycles = sourceCap(searchSettingCycleSource(query));
      const settingsResults = sourceCap(searchSettingsSource(query));
      const files =
      ...
      this.results = capResultsForRender(this.results, MERGED_RESULT_LIMIT);
  ...
  private async enterInFile(): Promise<void> {
    const sel = this.results[this.selected];
    if (!sel || sel.source !== 'file' || !sel.filePath || !canSearchInsideFile(sel)) return;
    const electron = getElectronHost();
    if (!electron) return;
  ...
  private renderHints(): void {
    ...
    if (!inFile) {
      if (sel?.source === 'file') {
        if (canSearchInsideFile(sel)) segs.push('⇥ search inside');
    ...
    this.results.forEach((r, i) => {
      ...
      if (r.source === 'file' && r.filePath) {
        ...
        row.addEventListener('contextmenu', (ev) => {
          ev.preventDefault();
          this.selected = i;
          if (canSearchInsideFile(r)) void this.enterInFile();
        });
      }
      row.appendChild(top);
