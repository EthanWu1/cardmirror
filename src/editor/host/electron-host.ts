...
interface ElectronAPI {
  ...
  saveExisting(handle: string, bytes: Uint8Array, opts?: SaveExistingOptions): Promise<SaveExistingResult>;
  saveSendDoc(
  ...
  windowCommand(action: 'minimize' | 'maximize-or-restore' | 'close'): Promise<void>;
...
export class ElectronHost implements Host {
  ...
  async chooseSaveLocation(
    ...
    const result = await api().chooseSaveLocation(suggestedName, {
      filters: opts.filters ?? [],
      ...(opts.nearPath ? { nearPath: opts.nearPath } : {}),
    });
    if (!result) return null;
  ...
  async saveExisting(
    ...
    return await api().saveExisting(handle, bytes, opts);
  }
  ...
  /** Cached, persisted recursive openable editor-file listing for the file-search
  ...
  async windowCommand(action: 'minimize' | 'maximize-or-restore' | 'close'): Promise<void> {
    await api().windowCommand(action);
  }
