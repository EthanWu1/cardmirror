...
function mimeForExtension(ext: string): string {
  ...
  if (ext === 'cmir' || ext === 'cmflow' || ext === 'flowline.json') return 'application/json';
...
export class BrowserHost implements Host {
  ...
  async windowCommand(_action: 'minimize' | 'maximize-or-restore' | 'close'): Promise<void> {
    // Browser/PWA hosts use the browser's own chrome.
  }
