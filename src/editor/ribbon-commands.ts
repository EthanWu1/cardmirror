...
export const RIBBON_COMMAND_LABELS: Record<RibbonCommandId, string> = {
  ...
  startDragSelection: 'Start Drag Selection',
  toggleBold: 'Bold',
  toggleItalic: 'Italic',
  ...
  sendToFlowColumn: 'Send to Excel Flow (Legacy, one cell per line)',
  sendToFlowCell: 'Send to Excel Flow (Legacy, single cell)',
  sendHeadingsToFlowColumn: 'Send Headings to Excel Flow (Legacy, one cell per line)',
  sendHeadingsToFlowCell: 'Send Headings to Excel Flow (Legacy, single cell)',
  pullFromFlow: 'Pull Selection from Excel Flow (Legacy)',
...
export const RIBBON_COMMAND_ALIASES: Partial<Record<RibbonCommandId, readonly string[]>> = {
  ...
  startDragSelection: ['drag', 'drag cards', 'move cards', 'move selection', 'send multiple cards'],
  ...
  saveMarkedCards: ['mark card', 'mark cards', 'marked cards', 'extract marked cards', 'export marked cards', 'save marked'],
  openEvidenceSearch: ['evidence search', 'search evidence files', 'search cards', 'corpus search'],
...
export const DEFAULT_RIBBON_KEYS: Record<RibbonCommandId, string | string[]> = {
  ...
  addAffFlow: '',
  addNegFlow: '',
  openFlow: 'Mod-Shift-o',
  importFlowlineJson: '',
  exportFlowlineJson: '',
  flowFind: 'Mod-Alt-f',
  flowSaveNow: 'Mod-Alt-Shift-s',
  createLegacyExcelFlow: '',
  startFlowHost: '',
...
export interface RibbonContext {
  ...
  /** Open the evidence search palette over file-search folders. */
  ...
  openTableMenu: () => void;
  /** Arm editor structural drag mode without requiring the pickup modifier chord. */
...
function commandFor(id: RibbonCommandId, ctx: RibbonContext): Command {
  switch (id) {
    ...
    case 'startDragSelection':
      return (_state, dispatch) => {
        if (!dispatch) return true;
        ctx.startDragSelection();
        return true;
      };
...
export type ViewlessRibbonCommandId =
  | 'createFlow'
  | 'addAffFlow'
  | 'addNegFlow'
  | 'openFlow'
...
const VIEWLESS_RIBBON_COMMAND_IDS = new Set<RibbonCommandId>([
  'createFlow',
  'addAffFlow',
  'addNegFlow',
  'openFlow',
...
export function runRibbonCommand(
  id: ViewlessRibbonCommandId,
  ctx: RibbonContext = DEFAULT_RIBBON_CONTEXT,
): boolean {
  if (!VIEWLESS_RIBBON_COMMAND_IDS.has(id)) {
    throw new Error(`runRibbonCommand only supports viewless commands; received ${id}.`);
  }
  return commandFor(id, ctx)({} as EditorState, () => {}, undefined);
}
