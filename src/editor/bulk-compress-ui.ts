 ...
 *  (parse + re-serialize, preserving threads, docId, and sharing metadata),
 *  and Save-As the smaller copy. The web edition can't do the desktop in-place
 *  folder walk, so it works one file at a time. */
export function runCompressSingleFileWeb(): Promise<void> {
  return runWebFileTool({
    ...
    run: async (bytes, name) => {
      const { doc, threads, docId, sharedDoc } = parseNative(bytes);
