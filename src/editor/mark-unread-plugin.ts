...
export const markUnreadPerfProbe = {
  fullBuilds: 0,
  rangeBuilds: 0,
};
...
export function computeUnreadDecorations(doc: PMNode): Decoration[] {
  return computeUnreadDecorationsInRange(doc, 0, doc.content.size);
}

function computeUnreadDecorationsInRange(doc: PMNode, from: number, to: number): Decoration[] {
  markUnreadPerfProbe.rangeBuilds += 1;
...
function build(doc: PMNode): DecorationSet {
  ...
  markUnreadPerfProbe.fullBuilds += 1;
...
export const markUnreadPlugin: Plugin<DecorationSet> = new Plugin<DecorationSet>({
  state: {
    ...
    apply(tr, prev, _old, newState) {
      ...
      if (tr.getMeta(MARK_UNREAD_TOGGLE) !== undefined) return build(newState.doc);
      if (!tr.docChanged) return prev;
      if (!settings.get('markUnreadAfterMarker')) return DecorationSet.empty;
      const range = changedRange(tr);
      if (!range) return prev.map(tr.mapping, newState.doc);
      const expanded = expandToTopLevel(newState.doc, range.from, range.to);
      const mapped = prev.map(tr.mapping, newState.doc);
      const stale = mapped.find(expanded.from, expanded.to);
      const fresh = computeUnreadDecorationsInRange(newState.doc, expanded.from, expanded.to);
