...
import { fromDocxFull, toDocx, serializeNative, serializeNativeAsync, parseNative, readDocIdFromBytes, stampDocId, type SharedDocMetadata } from '../index.js';
...
import { promptForText, promptForChoice, promptForRouteChoice, alertDialog, confirmDialog, confirmDialogDetailed } from './text-prompt.js';
import { confirmCloseUnsaved, locationLabelFromHandle } from './unsaved-close-dialog.js';
...
import {
  decodeModeSwitchTarget,
  decodeModeSwitchMarker,
  encodeModeSwitchMarker,
  modeSwitchReopenDocsForLayoutSwitch,
  modeSwitchDirtyMap,
  type ModeSwitchDoc,
} from './mode-switch.js';
import {
...
import { shouldHandleInitialDocPayload, shouldShowHomeAfterBlankBoot } from './boot-home.js';
...
import { recoveryJournalMatchesDisk } from './recovery-prune.js';
...
import { runSendToFlow, runPullFromFlow, runCreateLegacyExcelFlow, runStartFlowHost } from './flow-port.js';
...
import {
  cardMirrorClipboardSerializer,
  serializeCardMirrorClipboardHtml,
} from './clipboard-export.js';
...
// Web edition only: reveal + wire the GitHub button in the ribbon's
// right-hand grid (no-op under Electron).
...
function wireWindowButton(
  id: string,
  action: 'minimize' | 'maximize-or-restore' | 'close',
): void {
  const btn = document.getElementById(id) as HTMLButtonElement | null;
  if (!btn) return;
  btn.addEventListener('click', () => {
    void getHost().windowCommand(action);
  });
}
wireWindowButton('window-minimize-btn', 'minimize');
wireWindowButton('window-maximize-btn', 'maximize-or-restore');
wireWindowButton('window-close-btn', 'close');
...
let autosaveStateForActive: () => boolean = () =>
  settings.get('autosaveEnabled') || sharedDocAutosaveForced();
...
let multiDocSetSharedDocForUid:
  | ((uid: string, sharedDoc: SharedDocMetadata | null) => void)
  | null = null;
let multiDocPersistSharedDocForUid:
  | ((uid: string, sharedDoc: SharedDocMetadata) => Promise<void> | void)
  | null = null;
let multiDocMarkDirtyForUid: ((uid: string) => void) | null = null;
...
export function enableMultiDocMode(opts: {
  ...
  onCreateFlow?: () => Promise<boolean> | boolean;
  onFlowOpen?: (opened: OpenedFile) => Promise<boolean> | boolean;
  saveFocusedFlow?: () => Promise<boolean | null> | boolean | null;
  saveFocusedFlowAs?: () => Promise<boolean | null> | boolean | null;
  ...
  /** Set the focused pane's body zoom exactly, used by smooth pinch zoom. */
  zoomFocusedTo?: (pct: number) => void;
  ...
  persistSharedDocForUid?: (uid: string, sharedDoc: SharedDocMetadata) => Promise<void> | void;
  ...
  multiDocOnCreateFlow = opts.onCreateFlow ?? null;
  multiDocOnFlowOpen = opts.onFlowOpen ?? null;
  multiDocSaveFocusedFlow = opts.saveFocusedFlow ?? null;
  multiDocSaveFocusedFlowAs = opts.saveFocusedFlowAs ?? null;
  multiDocAddFlowToFocused = opts.addFlowToFocused ?? null;
  ...
  multiDocZoomTo = opts.zoomFocusedTo ?? null;
  ...
  multiDocSetSharedDocForUid = opts.setSharedDocForUid ?? null;
  multiDocPersistSharedDocForUid = opts.persistSharedDocForUid ?? null;
  multiDocMarkDirtyForUid = opts.markDirtyForUid ?? null;
...
function connectOpenedSharedDoc(sharedDoc: SharedDocMetadata | null): void {
  const deps = { ...collabDeps, setDocTitle: undefined };
  void loadCollabUi()
    .then(async (m) => {
      try {
        if (sharedDoc) return await m.connectSharedDocFlow(deps, sharedDoc);
        const file = activeFile();
        if (file.format !== 'cmir') return false;
        if (typeof file.handle !== 'string' || !file.handle) return false;
        return await m.autoStartSharedDocFlow(deps);
      } catch (err) {
        console.error('Failed to connect shared document:', err);
        showToast(m.relayFailureMessage(err, {
          initiating: !sharedDoc,
          verb: sharedDoc ? 'connect shared document' : 'start co-editing',
        }));
        return false;
      }
    })
    .catch((err) => {
      console.error('Failed to connect shared document:', err);
      showToast(`Could not load co-editing: ${err instanceof Error ? err.message : String(err)}`);
    });
}
...
const ribbonContext: RibbonContext = {
  ...
  collabStartSession: () => {
    void loadCollabUi().then((m) => {
      const sharedDoc = activeSharedDoc();
      if (sharedDoc) {
        void m.connectSharedDocFlow({ ...collabDeps, setDocTitle: undefined }, sharedDoc);
        return;
      }
      void m.startSessionFlow(collabDeps);
    });
  },
  collabJoinSession: () => {
    ...
    void loadCollabUi().then((m) => {
      ...
      void m.joinSessionFlow(multiDocActive ? makeMultiPaneSessionDeps() : collabDeps);
    });
  },
  collabCopyShareCode: () => {
  ...
  aiAskAboutSelection: () => {
    ...
    learnStore.registerDoc({
      ...
      format: isDocumentFormat(f.format) ? f.format : null,
    });
  ...
  startDragSelection: () => {
    editorDragSurface.startCommandPickupMode();
  },
  sendToStarred: () => {
 ...
 *     ask first. Three-button confirm: Save → run the normal save
 *     flow, on success swap to a fresh doc; Don't save → swap immediately;
...
// ours (double zoom). Small deltas apply immediately so the document tracks
// the gesture instead of waiting for 10% thresholds. Non-passive so
// preventDefault takes effect; capture so we win before any inner scroll
// handler.
let gestureZoomSettleTimer = 0;
let gestureZoomAnchor: ViewportAnchor | null = null;
let gestureZoomLivePct = 100;
let gestureZoomActiveSession = false;

function applyGestureLiveZoom(): void {
  applyActiveLiveZoomTarget(gestureZoomLivePct);
  if (gestureZoomAnchor) restoreViewportAnchor(gestureZoomAnchor);
}

function beginGestureZoom(): void {
  if (gestureZoomActiveSession) return;
  gestureZoomActiveSession = true;
  gestureZoomLivePct = clampLiveZoom(zoomStateForActive());
  if (view && !activeFlowWorkspace) gestureZoomAnchor = captureViewportAnchor(view);
}
function finishGestureZoom(): void {
  gestureZoomSettleTimer = 0;
  if (gestureZoomAnchor) restoreViewportAnchor(gestureZoomAnchor);
  gestureZoomAnchor = null;
  gestureZoomActiveSession = false;
}
function scheduleGestureZoomFinish(): void {
  if (gestureZoomSettleTimer) window.clearTimeout(gestureZoomSettleTimer);
  gestureZoomSettleTimer = window.setTimeout(finishGestureZoom, GESTURE_ZOOM_SETTLE_MS);
}
function applyGestureZoomDelta(deltaY: number): void {
  if (!Number.isFinite(deltaY)) return;
  beginGestureZoom();
  gestureZoomLivePct = zoomPctAfterWheelDelta(gestureZoomLivePct, deltaY);
  applyGestureLiveZoom();
  scheduleGestureZoomFinish();
}
window.addEventListener(
  ...
  (e: WheelEvent) => {
    ...
    e.preventDefault();
    applyGestureZoomDelta(e.deltaY);
  },
...
let multiDocZoomTo: ((pct: number) => void) | null = null;
...
let zoomStateForActive: () => number = () =>
  activeFlowWorkspace?.getRound().settings.zoomPercent ?? liveZoomPct;
...
/** Set the window's live body zoom without desktop step-10 rounding. */
export function setLiveZoomPct(pct: number): void {
  liveZoomPct = clampLiveZoom(pct);
  applyZoom(liveZoomPct);
}
...
function updateZoomStatus(pct: number): void {
  zoomPct.textContent = `${Math.round(pct)}%`;
  zoomResetBtn.disabled = Math.abs(pct - 100) < 0.01;
}
...
// Buttons and keybindings still use 10% steps with full anchoring. Pinch /
// Ctrl-scroll uses live fractional targets and restores the viewport anchor
// each animation frame so text size, wrapping, and spacing all track the
// gesture without a delayed reflow.
...
function applyActiveLiveZoomTarget(target: number): void {
  if (!multiDocActive && activeFlowWorkspace) {
    activeFlowWorkspace.setZoom(target);
    updateZoomStatus(activeFlowWorkspace.getRound().settings.zoomPercent);
  } else if (multiDocActive && multiDocZoomBy) {
    if (multiDocZoomTo) multiDocZoomTo(target);
    else multiDocZoomBy(target - zoomStateForActive());
    updateZoomStatus(target);
  } else {
    setLiveZoomPct(target);
  }
}

function zoomActiveLiveTo(pct: number): void {
  const target = clampLiveZoom(pct);
  const anchor = view ? captureViewportAnchor(view) : null;
  applyActiveLiveZoomTarget(target);
  if (anchor) restoreViewportAnchor(anchor);
}

function zoomActiveStepTo(pct: number): void {
  zoomActiveLiveTo(clampZoom(pct));
}

/** Zoom the ACTIVE editor by a delta (single-pane the window, multi-pane
 *  the focused pane). */
function zoomActiveBy(deltaPct: number): void {
  zoomActiveStepTo(zoomStateForActive() + deltaPct);
}
...
function zoomActiveReset(): void {
  ...
  } else if (activeFlowWorkspace) {
    activeFlowWorkspace.setZoom(100);
    updateZoomStatus(activeFlowWorkspace.getRound().settings.zoomPercent);
  } else {
    setZoom(100);
  }
...
function applyPillVisibility(): void {
  ...
  // The bottom scroll runway is gated on the floating tray. Send/Receive live
  // in the status bar, so only the dropzone can cover document content. Applies
  // in both single-pane (#editor) and multi-pane (the anchored pane) — see the
...
settings.subscribe((s) => {
  ...
  applyFormattingPanel(s.formattingPanelMode, stylePreview, s.showCharacterStyles);
...
function initRibbonResizer(): void {
  ...
  const panelIds: string[][] = [
    ...
    ['settings-btn'],            // (k) Settings. Genuinely last —
...
// the current ribbonTooltipMode.
{
  ...
  const windowMinimizeEl = byId('window-minimize-btn');
  if (windowMinimizeEl) registerRibbonTooltip({ el: windowMinimizeEl, label: 'Minimize' });
  const windowMaximizeEl = byId('window-maximize-btn');
  if (windowMaximizeEl) registerRibbonTooltip({ el: windowMaximizeEl, label: 'Maximize or restore' });
  const windowCloseEl = byId('window-close-btn');
  if (windowCloseEl) registerRibbonTooltip({ el: windowCloseEl, label: 'Close' });
...
function runViewlessRibbon(id: RibbonCommandId): void {
  switch (id) {
    ...
    case 'createFlow': ribbonContext.createFlow(); return;
    case 'addAffFlow': ribbonContext.addAffFlow(); return;
    case 'addNegFlow': ribbonContext.addNegFlow(); return;
    case 'openFlow': ribbonContext.openFlow(); return;
    case 'importFlowlineJson': ribbonContext.importFlowlineJson(); return;
    case 'exportFlowlineJson': ribbonContext.exportFlowlineJson(); return;
    case 'flowFind': ribbonContext.flowFind(); return;
    case 'flowSaveNow': ribbonContext.flowSaveNow(); return;
    case 'createLegacyExcelFlow': ribbonContext.createLegacyExcelFlow(); return;
...
let currentSharedDoc: SharedDocMetadata | null = null;
...
function activeContentDirty(): boolean {
...
function mountActiveFlowWorkspace(opts: {
  ...
  activeFlowWorkspace = createFlowWorkspace({
    ...
    onChange: () => {
      activeFlowEditGen++;
      markNonPristineStarter();
      if (activeFlowIsDirty()) scheduleAutosaveAttempt();
      refreshAutosaveBtn();
    },
    onRequestSave: () => {
      void runSaveFlow();
  ...
  settings.set(
    'autosaveEnabled',
    opts.handle ? isAutosaveOnForPath(opts.handle, opts.format === 'cmflow') : settings.get('autosaveEnabled'),
  );
...
function activeSharedDoc(): SharedDocMetadata | null {
  if (multiDocActive && multiDocGetFocusedFile) {
    return multiDocGetFocusedFile()?.sharedDoc ?? null;
  }
  return currentSharedDoc;
}

function setSharedDocForUid(uid: string, sharedDoc: SharedDocMetadata | null): void {
  if (multiDocActive && multiDocSetSharedDocForUid) {
    multiDocSetSharedDocForUid(uid, sharedDoc);
    return;
  }
  if (uid === currentDocUid) currentSharedDoc = sharedDoc;
}

async function persistSharedDocForUid(
  uid: string,
  sharedDoc: SharedDocMetadata,
): Promise<void> {
  if (multiDocActive && multiDocPersistSharedDocForUid) {
    await multiDocPersistSharedDocForUid(uid, sharedDoc);
    return;
  }
  if (uid !== currentDocUid) return;
  const file = activeFile();
  if (file.format !== 'cmir') return;
  if (typeof file.handle !== 'string' || !file.handle) return;
  if (!getHost().supportsInPlaceSave) return;
  const docId = currentDocId ?? ensureActiveDocId();
...
function markDirtyForUid(uid: string): void {
  if (multiDocActive && multiDocMarkDirtyForUid) {
    multiDocMarkDirtyForUid(uid);
    return;
  }
  if (uid !== currentDocUid) return;
  markCurrentDocDirty();
...
async function stampActiveFileDocId(docId: string): Promise<void> {
  ...
  try {
    ...
    if (!read || !isDocumentFormat(read.format)) return;
    ...
    await getHost().saveExisting(f.handle, stamped, { force: f.format === 'cmir' });
  } catch (err) {
...
let currentDocCleanBaseline: PMNode | null = currentDoc;
...
function activeSingleDocSnapshot(): PMNode {
  return view?.state.doc ?? currentDoc;
}
function cancelPendingSingleDocPersistence(): void {
...
function markCurrentDocChanged(doc: PMNode): boolean {
  currentDocDirty = isMeaningfullyDirtyFromBaseline(currentDocCleanBaseline, doc, currentDocFormat);
  currentDocEditGen++;
  if (!currentDocDirty) cancelPendingSingleDocPersistence();
  return currentDocDirty;
}
function markCurrentDocClean(): void {
  currentDocDirty = false;
  currentDocCleanBaseline = activeSingleDocSnapshot();
...
function isDocumentFormat(format: unknown): format is 'cmir' | 'docx' {
...
const OPEN_FILE_FILTERS = [
  { name: 'CardMirror, Flow, Word, or recovery journal', extensions: ['cmir', 'cmflow', 'flowline.json', 'cmir-journal', 'docx'] },
...
async function runOpenFlow(opts: { beforeRoute?: () => void } = {}): Promise<boolean> {
  let opened: OpenedFile | null;
  try {
  ...
  if (!opened) return false;
  if (flowFormatForFilename(opened.name)) {
    const routed = await routeOpenedFlowFile(opened);
    if (routed) opts.beforeRoute?.();
    return routed;
  }
  opts.beforeRoute?.();
...
async function routeOpenedFile(opened: OpenedFile): Promise<void> {
  ...
  try {
    ...
    currentSharedDoc = sharedDoc;
    // Saved .cmir files autosave by default; .docx stays manual-save unless
    // the user explicitly enabled autosave for that path.
    settings.set('autosaveEnabled', isAutosaveOnForPath(src.handle, format === 'cmir'));
    currentDocUid = newSessionDocUid();
    ...
    if (!src.recovered) {
      ...
      connectOpenedSharedDoc(sharedDoc);
    }
    homeScreen.hide();
...
async function loadFileInPlace(file: {
  ...
  // Saved .cmir files autosave by default; .docx stays manual-save unless
  // the user explicitly enabled autosave for that path.
  settings.set('autosaveEnabled', isAutosaveOnForPath(file.handle, file.format === 'cmir'));
  currentDocUid = newSessionDocUid();
...
async function listHomeSearchFiles(): Promise<Array<{ path: string; relPath: string; mtimeMs: number; size?: number }>> {
  const electron = getElectronHost();
  if (!electron) return [];
  const roots = settings.get('fileSearchRoots');
  if (!roots.length) return [];
  const perRoot = await Promise.all(
    roots.map((root) => electron.listCmirFiles(root).catch(() => [])),
  );
  const byPath = new Map<string, { path: string; relPath: string; mtimeMs: number; size?: number }>();
...
const homeCallbacks: HomeScreenCallbacks = {
  // Single-doc: load in-place in this window. Multi-pane: pick first, then hide
  // Home only for a routeable file so deferred Flow opens leave Home visible.
  newDoc: () => {
    if (multiDocActive) {
      homeScreen.hide();
      void multiDocOnNewDoc?.();
  ...
  open: () => {
    if (multiDocActive) {
      ...
      void runOpenFlow({ beforeRoute: () => homeScreen.hide() });
      return;
    }
    void (async () => {
  ...
  openFilePath: (path: string, name: string) => {
    const flowPath = isFlowFilename(path) || isFlowFilename(name);
    if (flowPath) {
      void (async (): Promise<void> => {
        if (multiDocActive) homeScreen.hide();
        const opened = await openFileByPath(path, name, { forceFlowInPlace: true });
        if (opened) homeScreen.hide();
        else if (multiDocActive) homeScreen.show();
      })();
      return;
    }
    homeScreen.hide();
    void openFileByPath(path, name);
  },
  listSearchFiles: listHomeSearchFiles,
...
function mountFreshBlankDoc(): void {
  if (shouldClearFlowStateBeforeBlankDocMount(activeFlowWorkspace)) {
    destroyActiveFlowWorkspace();
  }
  void clearCurrentJournal();
  mountView(makeNewDocBody());
...
async function handleCloseDocToHomeInner(): Promise<void> {
  ...
  const co = activeFlowWorkspace
    ? 'run-normal'
    : await resolveCoEditedClose(currentDocUid, currentDocFilename ?? '');
  if (co === 'cancel') return;
  if (co === 'keep') {
  ...
  const choice = await confirmCloseUnsaved(activeCloseUnsavedOptions());
  switch (choice) {
    case 'save': {
...
async function pickAndLoadInPlace(): Promise<boolean> {
  ...
  if (!opened) return false;
  if (isFlowFilename(opened.name)) {
    return routeOpenedFlowFile(opened, { forceInPlace: true });
  }
  const src = resolveOpenedFile(opened);
  if (src === 'corrupt') {
    void alertDialog('That .cmir-journal file is corrupt or could not be read.');
...
async function openFileByPath(
  ...
  if (file.format === 'cmflow' || flowFormatForFilename(file.name) || flowFormatForFilename(name)) {
    return routeOpenedFlowFile(
      { name: file.name, bytes: file.bytes, handle: file.handle },
      { forceInPlace: opts.forceFlowInPlace },
    );
  }
...
function installDragToOpen(): void {
  const electron = getElectronHost();
  if (!electron) return;
  const SUPPORTED = /\.(cmir|cmflow|flowline\.json|cmir-journal|docx)$/i;
 ...
 *  window blank. Returns true only if the payload was handled. */
async function routeInitialDocIntoWorkspace(): Promise<boolean> {
  ...
  if (payload.format === 'cmflow' || isFlowFilename(payload.filename)) {
    if (multiDocActive) {
      return (await multiDocOnFlowOpen?.({
  ...
  if (!shouldHandleInitialDocPayload(payload)) {
    showToast('Native Flow opening will be available after the Flow workspace is wired.');
    return false;
  }
...
async function openRecentInPlace(recent: RecentFile): Promise<void> {
  ...
  if (file.format === 'cmflow' || flowFormatForFilename(file.name) || flowFormatForFilename(recent.filename)) {
    if (multiDocActive) {
      homeScreen.hide();
      await routeOpenedFlowFile({ name: file.name, bytes: file.bytes, handle: file.handle });
      return;
    }
    ...
    if (!isPristineStarter && electron.canSpawnWindow) {
      if (activeFlowHandle != null && (await isSameOpenHandle(activeFlowHandle, file.handle))) {
        homeScreen.hide();
        return;
      }
      homeScreen.hide();
    ...
    await mountOpenedFlowInPlace({ name: file.name, bytes: file.bytes, handle: file.handle });
    return;
  }
  const { takenByOther } = await electron.openPathCheck(file.handle);
  if (takenByOther) {
 ...
 *  module-level `currentDoc*` values. */
function activeFile(): { filename: string | null; handle: unknown | null; format: 'cmir' | 'docx' | 'cmflow' | null } {
  if (multiDocActive && multiDocGetFocusedFile) {
    const f = multiDocGetFocusedFile();
  ...
  if (activeFlowWorkspace) {
    return {
      filename: activeFlowFilename,
      handle: activeFlowHandle,
      format: activeFlowFormat === 'cmflow' ? 'cmflow' : null,
    };
  }
  return { filename: currentDocFilename, handle: currentDocHandle, format: currentDocFormat };
}

interface PendingCloseSaveTarget {
  uid: string;
  name: string;
  handle: string;
  format: 'cmir' | 'docx';
...
let pendingCloseSaveTarget: PendingCloseSaveTarget | null = null;

function suggestedSaveNameForActiveFile(): { filename: string; format: 'cmir' | 'docx' } {
  const file = activeFile();
  const format = isDocumentFormat(file.format) ? file.format : settings.get('defaultSaveFormat');
  const base = basenameWithoutExt(file.filename ?? 'untitled').trim() || 'untitled';
  return { filename: `${base}.${format}`, format };
}

export async function chooseCloseSaveLocationForActiveFile(): Promise<string | null> {
  const electron = getElectronHost();
  if (!electron) return null;
  const file = activeFile();
  const uid = activeDocIdentity().sessionUid;
  const suggestion = suggestedSaveNameForActiveFile();
  const result = await electron.chooseSaveLocation(suggestion.filename, {
    filters: saveFiltersForFormat(suggestion.format),
    ...(typeof file.handle === 'string' && file.handle ? { nearPath: file.handle } : {}),
  });
  if (!result || typeof result.handle !== 'string') return null;
  pendingCloseSaveTarget = {
    uid,
    name: result.name,
    handle: result.handle,
    format: formatFromFilename(result.name) ?? suggestion.format,
  };
  return locationLabelFromHandle(result.handle) ?? result.name;
}

export function clearCloseSaveLocationForActiveFile(): void {
  const uid = activeDocIdentity().sessionUid;
  if (pendingCloseSaveTarget?.uid === uid) pendingCloseSaveTarget = null;
}

function takeCloseSaveLocationForActiveFile(): PendingCloseSaveTarget | null {
  const uid = activeDocIdentity().sessionUid;
  if (pendingCloseSaveTarget?.uid !== uid) return null;
  const target = pendingCloseSaveTarget;
  pendingCloseSaveTarget = null;
  return target;
}

function sameDesktopPath(a: string, b: string): boolean {
  const normalize = (value: string): string =>
    value.replace(/[\\/]+/g, '\\').replace(/\\+$/g, '').toLowerCase();
  return normalize(a) === normalize(b);
}

function activeCloseUnsavedOptions(): { locationLabel?: string | null; onChooseLocation?: () => Promise<string | null> } {
  if (activeFlowWorkspace) {
    return {
      locationLabel: locationLabelFromHandle(activeFlowHandle),
    };
  }
  return {
    locationLabel: locationLabelFromHandle(activeFile().handle),
    onChooseLocation: chooseCloseSaveLocationForActiveFile,
  };
}
...
function commitSaveResult(filename: string, handle: unknown | null, format: 'cmir' | 'docx'): void {
  if (multiDocActive && multiDocSetFocusedFile) {
    ...
    currentDocFormat = format;
    settings.set('autosaveEnabled', isAutosaveOnForPath(handle, format === 'cmir'));
  }
  updateWindowTitle();
...
async function serializeForSave(
  ...
  docId?: string,
  /** Persistent collaboration pointer. Only embedded in `.cmir` working saves. */
  sharedDoc?: SharedDocMetadata | null,
): Promise<Uint8Array> {
  ...
  if (format === 'cmir') {
    ...
    return serializeNativeAsync(exportDocNode, {
      ...threadsOpt,
      ...(docId ? { docId } : {}),
      ...(sharedDoc ? { sharedDoc } : {}),
    });
...
function ensureCmflowFilename(name: string): string {
  return /\.cmflow$/i.test(name) ? name : `${name.replace(/\.+$/, '')}.cmflow`;
}

function activeFlowBytes(round: FlowRound): Uint8Array {
  return serializeFlowFile({
    round,
    createdAt: activeFlowCreatedAt ?? round.createdAt,
    updatedAt: new Date().toISOString(),
  });
}

function commitActiveFlowSaveResult(filename: string, handle: unknown | null, commitClean: () => boolean): void {
  activeFlowFilename = filename;
  activeFlowFormat = 'cmflow';
  setActiveFlowHandle(handle);
  commitClean();
  settings.set('autosaveEnabled', isAutosaveOnForPath(handle, true));
  recordRecent({
    handle: typeof handle === 'string' ? handle : null,
    filename,
    format: 'cmflow',
...
async function runActiveFlowSaveAs(): Promise<boolean> {
  const round = activeFlowRound();
  if (!round) return false;
  const suggestedName = ensureCmflowFilename(
    suggestedFlowSaveName(activeFlowFilename ?? defaultFlowName(round.format)),
  );
  try {
    const commitClean = captureActiveFlowCleanToken();
    const roundForSave = activeFlowRound();
    if (!roundForSave) return false;
    const bytes = activeFlowBytes(roundForSave);
    const result = await getHost().saveAs(suggestedName, bytes, {
      filters: CMFLOW_SAVE_FILTERS,
      ...(typeof activeFlowHandle === 'string' && activeFlowHandle ? { nearPath: activeFlowHandle } : {}),
    });
    if (!result) return false;
    commitActiveFlowSaveResult(result.name, result.handle ?? null, commitClean);
    flashSaveSuccess();
    return true;
  } catch (err) {
    console.error('Flow Save As failed:', err);
    void alertDialog(`Flow Save As failed: ${err instanceof Error ? err.message : err}`);
    return false;
  }
}

async function runActiveFlowSave(): Promise<boolean> {
  if (!activeFlowWorkspace) return false;
  if (!activeFlowHandle || activeFlowFormat !== 'cmflow' || !getHost().supportsInPlaceSave) {
    return runActiveFlowSaveAs();
  }
  if (!(await getHost().ensureWritable(activeFlowHandle))) {
    return runActiveFlowSaveAs();
  }
  try {
    const commitClean = captureActiveFlowCleanToken();
    ...
    if (isFileGoneError(err)) {
      const rescue = await confirmDialog(
        `"${activeFlowFilename ?? 'This Flow'}" couldn't be saved because its file no longer exists at the saved location. Choose a new location to keep your work.`,
        { title: 'File location not found', okLabel: 'Save As...' },
      );
      return rescue ? runActiveFlowSaveAs() : false;
    }
...
async function runSaveAsFlowInner(): Promise<boolean> {
  if (multiDocActive && multiDocSaveFocusedFlowAs) {
    const flowSaved = await multiDocSaveFocusedFlowAs();
    if (flowSaved !== null) return flowSaved;
  }
  if (activeFlowWorkspace) return runActiveFlowSaveAs();
  const file = activeFile();
  ...
  const defaultFormat: 'cmir' | 'docx' = isDocumentFormat(file.format)
    ? file.format
    : settings.get('defaultSaveFormat');
  ...
  try {
    ...
    const sharedDocForSave =
      isFullSave && !hadOnDiskOriginal && choice.format === 'cmir'
        ? activeSharedDoc()
        : null;
    ...
    if (isFullSave) {
      ...
      if (!sharedDocForSave) {
        setSharedDocForUid(sessionUid, null);
      }
      if (forkDocId) {
...
async function runSaveToChosenLocation(target: PendingCloseSaveTarget): Promise<boolean> {
  const file = activeFile();
  if (
    typeof file.handle === 'string' &&
    file.format === target.format &&
    sameDesktopPath(file.handle, target.handle)
  ) {
    return runSaveFlowInner();
  }
  if (target.format === 'docx' && !(await confirmDocxDropsLiveLinks())) return false;
  const electron = getElectronHost();
  if (!electron) return runSaveAsFlow();
  const hadOnDiskOriginal = file.handle != null;
  const forkDocId = crypto.randomUUID();
  const sharedDocForSave =
    !hadOnDiskOriginal && target.format === 'cmir'
      ? activeSharedDoc()
      : null;
  try {
    const commitClean = captureActiveDocCleanToken();
    ...
    await electron.writeFileAtPath(target.handle, bytes);
    const { docId: srcDocId, sessionUid } = activeDocIdentity();
    commitSaveResult(target.name, target.handle, target.format);
    if (!sharedDocForSave) {
      setSharedDocForUid(sessionUid, null);
    }
    if (srcDocId && hadOnDiskOriginal) {
...
async function runSaveFlowInner(): Promise<boolean> {
  if (multiDocActive && multiDocSaveFocusedFlow) {
    const flowSaved = await multiDocSaveFocusedFlow();
    if (flowSaved !== null) return flowSaved;
  }
  if (activeFlowWorkspace) return runActiveFlowSave();
  const chosenTarget = takeCloseSaveLocationForActiveFile();
  if (chosenTarget) {
    return runSaveToChosenLocation(chosenTarget);
  }
  const file = activeFile();
  ...
  if (documentFormat === 'docx' && !(await confirmDocxDropsLiveLinks())) return false;
  try {
    ...
    let saveResult: SaveExistingResult | null = null;
    try {
      saveResult = await getHost().saveExisting(file.handle, bytes, {
        force: documentFormat === 'cmir',
        conflictCopiesForDocx: documentFormat === 'docx',
      });
    } catch (err) {
      ...
      saveResult = await getHost().saveExisting(file.handle, bytes, { force: true });
    }
    if (saveResult?.status === 'conflict') {
      commitSaveResult(saveResult.conflictName, saveResult.conflictPath, documentFormat);
      showToast(`Saved separate copy: ${saveResult.conflictName}`, { durationMs: 7000 });
      void alertDialog(
        `Someone changed this Word file outside CardMirror, so your edits were saved as a separate copy.\n\nNow open:\n${saveResult.conflictName}`,
        { title: 'Saved separate copy' },
      );
    }
    ...
    if (docId) {
      learnStore.registerDoc({
        docId,
        path: typeof savedFile.handle === 'string' ? savedFile.handle : null,
        name: savedFile.filename ?? 'Untitled',
    ...
    if (isFileBusyError(err)) {
      void alertDialog(
        `"${file.filename ?? 'This document'}" couldn't be saved because another app or sync service is using the file.\n\nClose it in Word, or wait for Dropbox to finish syncing, then save again.`,
        { title: 'File is busy' },
      );
      return false;
    }
...
function scheduleAutosaveAttempt(): void {
  ...
  autosaveTimer = window.setTimeout(() => {
    ...
    autosaveChain = autosaveChain
      ...
      .catch((err) => reportAutosaveFailure(activeFile().filename ?? 'Untitled', err));
  }, delay);
}
...
async function runJournalWrite(): Promise<void> {
  ...
  try {
    const bytes = await serializeNativeAsync(view.state.doc, {
      ...
      ...(currentSharedDoc ? { sharedDoc: currentSharedDoc } : {}),
    });
...
async function runAutosaveAttempt(): Promise<void> {
  if (!settings.get('autosaveEnabled') && !sharedDocAutosaveForced()) return;
  if (activeFlowWorkspace) {
    if (activeFlowFormat !== 'cmflow' || !activeFlowHandle) return;
    if (!getHost().supportsInPlaceSave) return;
    try {
      ...
      reportAutosaveFailure(activeFlowFilename ?? 'Untitled Flow', err);
    }
    return;
  }
  if (!activeSharedDocDiskWriter()) return;
...
function refreshAutosaveBtn(): void {
  ...
  if (!on) {
    ...
    const file = activeFile();
    const effective = (file.format === 'cmir' || file.format === 'cmflow') && !!file.handle;
    autosaveBtn.dataset['autosaveEffective'] = effective ? 'true' : 'false';
    if (effective && file.format === 'cmflow') {
      label = 'Autosave is on — saves to .cmflow every few seconds after edits';
    } else if (effective) {
      ...
      label = 'Autosave is on, but this Flow has not been saved as .cmflow yet. Save once to enable.';
    } else {
...
}
{
  ...
  if (electronHost) {
    ...
    // close immediately. If dirty, prompt for save / discard / cancel.
 ...
 *  immediately; dirty docs surface a compact save prompt and act on
...
export { confirmCloseUnsaved, locationLabelFromHandle } from './unsaved-close-dialog.js';
...
function confirmCloseCoEditedDoc(
  ...
  const detail =
    info.unsynced > 0
      ? `${info.unsynced} unsynced change${info.unsynced === 1 ? '' : 's'} will sync when you reopen.`
      : undefined;
...
// Single cross-window dropzone pill, anchored to the editor's bottom-left
// corner (NOT the nav pane — dragging onto a nav-bottom pill scrolled the
// outline). `positionDropzone` tracks the editor element's rect, so it follows
// nav-width changes, the status bar, and (multi-pane) the leftmost pane.
...
if (!BOOT_MOBILE) {
  // The floating tray now holds only the dropzone. Send / Receive live in the
  // status bar beside the read-time display so they don't cover the document.
  ...
  const pairingStatusControls = document.getElementById('pairing-status-controls');
  mountPairingPills(pairingStatusControls ?? pillTray, () => getActiveView());
  initPairingWiring();
}
...
function positionDropzone(): void {
  // Anchors the floating dropzone tray. Kept the name for its existing call
  // sites (rAF / resize / ResizeObserver hooks).
...
async function initSingleDocBoot(): Promise<void> {
  ...
  if (shouldShowHomeAfterBlankBoot({ isFirstWindow: isFirst, modeSwitchPending })) {
    // Launched with no file: show Home over the disposable starter doc.
    // Recovery still runs only for the first window / mode-switch owner;
    // later blank windows should also land on Home, not an empty editor.
    homeScreen.show();
  }
...
async function mountFromSpawnPayload(
  ...
  try {
    ...
    let sharedDoc: SharedDocMetadata | null = null;
    const format =
      (isDocumentFormat(payload.format) ? payload.format : null) ??
      formatFromFilename(payload.filename) ??
      'docx';
    ...
    markNonPristineStarter();
    updateWindowTitle();
    if (!payload.markDirty) connectOpenedSharedDoc(sharedDoc);
...
async function handleModeSwitchInner(newValue: boolean): Promise<void> {
  ...
  if (newValue) {
    message = electron
      ? 'Any other open CardMirror windows will close. The document shown here will reopen in slot 1.'
      : 'The document shown here will reopen in slot 1. Your other CardMirror windows will ask you to close them.';
  } else {
    message = electron
  ...
  let confirmed = true;
  if (settings.get('multiDocConfirmModeSwitch')) {
    const choice = await confirmDialogDetailed(message, {
      title: newValue
    ...
    if (choice.confirmed && choice.checked) {
      settings.set('multiDocConfirmModeSwitch', false);
    }
  }
  if (!confirmed) {
  ...
  try {
    ...
    // on every toggle.
    const markerDocs = modeSwitchReopenDocsForLayoutSwitch(reopenDocs, remoteDocs, newValue);
    sessionStorage.setItem(
      MODE_SWITCH_MARKER_KEY,
      encodeModeSwitchMarker(markerDocs, { toMultiDoc: newValue }),
    );
    console.log(
      `[cardmirror] modeswitch: journaled ${reopenDocs.length} local + ${remoteDocs.length} remote doc(s), reopening ${markerDocs.length}, switching to ${newValue ? 'multi' : 'single'}-pane`,
    );
  } catch (err) {
...
async function runStartupRecoveryInner(): Promise<void> {
  ...
  const markerRaw = sessionStorage.getItem(MODE_SWITCH_MARKER_KEY);
  const markerDocs = decodeModeSwitchMarker(markerRaw);
  if (markerDocs !== null) {
    ...
    const targetIsMultiDoc = decodeModeSwitchTarget(markerRaw) === true;
    let reportedRemoteDocs: ModeSwitchDoc[] = [];
    try {
      reportedRemoteDocs = (await getElectronHost()?.takeModeSwitchJournaledDocs()) ?? [];
    } catch (err) {
      console.warn('Failed to collect mode-switch doc reports:', err);
    }
    if (targetIsMultiDoc) {
      // Single-window -> three-pane now reopens only this window's current doc.
      // Still clear clean reports from other windows so they do not become
      // fake crash-recovery drafts later; leave dirty reports recoverable.
      await Promise.all(
        reportedRemoteDocs
          .filter((d) => !d.dirty)
          .map((d) => host.deleteJournal(d.uid).catch(() => {})),
    ...
    const remoteDocs = targetIsMultiDoc ? [] : reportedRemoteDocs;
  ...
  entries = await pruneRedundantRecoveryJournals(entries, host);
    ...
    onOpen: async (entry) => {
      ...
      if (multiDocActive && multiDocOnRecoveredDoc) {
        try {
          ...
          const entryFormat = isDocumentFormat(entry.format) ? entry.format : null;
...
async function pruneRedundantRecoveryJournals(
  entries: JournalEntry[],
  host: ReturnType<typeof getHost>,
): Promise<JournalEntry[]> {
  const electron = getElectronHost();
  if (!electron) return entries;
  ...
  for (const entry of entries) {
    ...
    try {
      const disk = await electron.readFileAtPath(entry.handle);
      if (!disk) {
        kept.push(entry);
        continue;
      }
      const format = isDocumentFormat(entry.format)
        ? entry.format
        : isDocumentFormat(disk.format)
          ? disk.format
          : null;
      if (!format) {
        kept.push(entry);
      ...
      const redundant = await recoveryJournalMatchesDisk(
        entry.bytes,
        disk.bytes,
        format,
      );
      if (redundant) {
        await host.deleteJournal(entry.uid);
        console.log(`[cardmirror] recovery: dropped redundant journal for "${entry.filename}"`);
      } else {
        kept.push(entry);
      }
    } catch (err) {
      console.warn(`Failed to compare recovery journal for ${entry.uid}:`, err);
      kept.push(entry);
    }
  }
  return kept;
...
async function saveRecoveryEntry(entry: JournalEntry): Promise<boolean> {
  ...
  if (entry.handle && entryFormat && host.supportsInPlaceSave) {
    try {
      const bytes = await reserializeJournalAs(entry, entryFormat);
      if (typeof entry.handle !== 'string') return false;
      await host.saveExisting(entry.handle, bytes, { force: entryFormat === 'cmir' });
  ...
  const choice = await openSaveAs({
    ...
    defaultFormat: entryFormat ?? 'cmir',
  });
  if (!choice) return false;
  try {
...
async function applyRecovery(
  ...
  currentDocFormat = entryFormat;
  currentSharedDoc = parsed.sharedDoc;
  ...
  adoptDocId(parsed.docId, entry.filename, entry.handle, entryFormat);
