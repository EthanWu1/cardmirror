...
class SettingsModal {
  ...
  private renderEntry(meta: SettingMeta): HTMLElement {
    ...
    if (meta.kind === 'toggle') {
      ...
      label.appendChild(buildNumberEditor(meta.key as keyof Settings, meta.min, meta.max, meta.step));
    } else if (meta.kind === 'select') {
      row.appendChild(text);
      row.appendChild(buildSelectEditor(meta));
      return row;
  ...
  input.addEventListener('change', () => {
    ...
    const stepped =
      step > 0 && step !== 1
        ...
        : Math.round(parsed);
    const v = Math.max(min, max === undefined ? stepped : Math.min(max, stepped));
    if (!Number.isFinite(v)) {
      input.value = String(settings.get(key));
      return;
    }
...
function buildSelectEditor(meta: SettingMeta): HTMLElement {
  const select = document.createElement('select');
  select.className = 'pmd-body-font-select';
  for (const option of meta.options ?? []) {
    ...
    if (option.value === settings.get(meta.key)) opt.selected = true;
    select.appendChild(opt);
  }
  select.addEventListener('change', () => {
    settings.set(meta.key, select.value as never);
  });
  const unsubscribe = settings.subscribe(() => {
    const current = String(settings.get(meta.key));
    if (select.value !== current) select.value = current;
  });
  registerRowCleanup(select, () => unsubscribe());
  return select;
}
