...
function docHasLocalContent(view: EditorView | null): boolean {
  return !!view && view.state.doc.textContent.trim().length > 0;
}

async function confirmSharedDocReplacement(deps: CollabUiDeps): Promise<boolean> {
  if (!docHasLocalContent(deps.getView())) return true;
  return confirmDialog(
    'The live shared-document room will replace the local file contents. If you edited this file offline and do not have a saved session record, keep the local copy and save it separately.',
    {
      title: 'Connect shared document?',
      okLabel: 'Connect Shared Document',
...
export async function connectSharedDocFlow(
  ...
  if (record) {
    const ok = await resumeSessionFlow(deps, decoded.roomId, { existingDoc: true });
    if (ok) {
      setSharedDocOpenState(ownerUid, null);
      deps.setSharedDocForUid?.(ownerUid, sharedDoc);
    } else {
  ...
  try {
    ...
    if (!(await confirmSharedDocReplacement(deps))) {
      await session.stop();
      markLocalCopy('Kept local copy. Use Join Collaboration Session to try reconnecting again.');
      showToast('Shared document kept as a local copy');
      deps.getView()?.focus();
      return false;
    }
