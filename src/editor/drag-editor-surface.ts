...
export class EditorDragSurface implements DragSurface {
  ...
  private pickupCommandArmed = false;
  private hovered: HoveredContainer | null = null;
  private highlightBox: HTMLElement | null = null;
  private selectedContainers: DragSelectionCandidate[] = [];
  private selectedBoxes: HTMLElement[] = [];
  private pendingSelectionDrag: {
    startX: number;
    startY: number;
  ...
  private boundOnDocumentPointerDown = (e: PointerEvent) => this.onDocumentPointerDown(e);
  ...
  startCommandPickupMode(): boolean {
    if (!this.view || !this.host || dragController.isActive()) return false;
    this.pickupCommandArmed = !this.pickupCommandArmed;
    if (this.pickupCommandArmed) {
  ...
  private onKey(e: KeyboardEvent): void {
    if (e.key === 'Escape' && (this.pickupCommandArmed || this.dragOriginatedHere)) {
      e.preventDefault();
      this.pickupCommandArmed = false;
      this.pickupModifierHeld = false;
      this.removeHighlight();
    ...
    if (nowHeld) {
      ...
      if (!this.pickupCommandArmed && dragController.isActive() && this.dragOriginatedHere) {
        dragController.cancel();
      }
    }
    this.applyPickupClass();
  ...
  private pickupModeActive(): boolean {
    return this.pickupModifierHeld || this.pickupCommandArmed;
  }

  private onBlur(): void {
    if (!this.pickupModeActive()) return;
    this.pickupModifierHeld = false;
    this.pickupCommandArmed = false;
    this.removeHighlight();
    this.hovered = null;
  ...
  private applyPickupClass(): void {
    if (!this.host) return;
    const inPickup = this.pickupModeActive() && !dragController.isActive();
  ...
  private onHostPointerDown(e: PointerEvent): void {
    ...
    if (this.pickupCommandArmed && !this.pickupModifierHeld) {
      this.handlePersistentSelectionPointerDown(e, container);
      return;
    }

    const fallback: DragSelectionCandidate = {
      from: container.from,
      to: container.to,
      id: null,
      type: container.type,
    ...
    const items = dragItemsForSelection(this.view.state.doc, this.view.state.selection, fallback);
  ...
  private onDocumentPointerDown(e: PointerEvent): void {
    if (!this.pickupCommandArmed || dragController.isActive() || this.pendingSelectionDrag) return;
    if (this.host && e.target instanceof Node && this.host.contains(e.target)) return;
    this.pickupCommandArmed = false;
    this.clearPersistentSelection();
    this.removeHighlight();
    this.hovered = null;
  ...
  private handlePersistentSelectionPointerDown(e: PointerEvent, container: HoveredContainer): void {
    const candidate = this.containerToCandidate(container);
    if (!this.isPersistentSelected(candidate)) {
      this.selectedContainers.push(candidate);
      this.renderSelectionHighlights();
    }
    this.pendingSelectionDrag = {
      startX: e.clientX,
      startY: e.clientY,
      fallback: candidate,
    };
  ...
  private beginPersistentSelectionDrag(clientX: number, clientY: number): void {
    if (!this.view || !this.pendingSelectionDrag) return;
    const items = dragItemsForPersistentSelection(
      this.selectedContainers,
      this.pendingSelectionDrag.fallback,
    );
    this.dragOriginatedHere = true;
    this.removeHighlight();
    this.hovered = null;
    ...
    this.applyPickupClass();
    this.createPickupPill(items);
    this.updatePickupPill(clientX, clientY);
    dragController.dispatchHit(clientX, clientY);
  }
  ...
  private onDocPointerMoveDuringDrag(e: PointerEvent): void {
    if (this.pendingSelectionDrag && !dragController.isActive()) {
      const dx = e.clientX - this.pendingSelectionDrag.startX;
      const dy = e.clientY - this.pendingSelectionDrag.startY;
      if (Math.hypot(dx, dy) < 6) return;
      this.beginPersistentSelectionDrag(e.clientX, e.clientY);
    }
    if (!dragController.isActive()) return;
  ...
  private onDocPointerUpDuringDrag(_e: PointerEvent): void {
    if (this.pendingSelectionDrag && !dragController.isActive()) {
      this.pendingSelectionDrag = null;
      this.detachDragListeners();
      return;
    }
    if (!dragController.isActive()) return;
  ...
  private containerToCandidate(container: HoveredContainer): DragSelectionCandidate {
    return {
      from: container.from,
      to: container.to,
      id: null,
  ...
  private isPersistentSelected(candidate: DragSelectionCandidate): boolean {
    return this.selectedContainers.some(
      (item) =>
        item.from === candidate.from &&
        item.to === candidate.to &&
        item.type === candidate.type,
    );
  }

  private clearPersistentSelection(): void {
    this.pendingSelectionDrag = null;
    this.selectedContainers = [];
  ...
  private renderSelectionHighlights(): void {
    this.removeSelectionHighlights();
    if (!this.view || !this.host) return;
    for (const item of this.selectedContainers) {
      try {
        const fromCoords = this.view.coordsAtPos(item.from);
        const toCoords = this.view.coordsAtPos(Math.max(item.from, item.to - 1));
        ...
        box.className = 'pmd-editor-pickup-highlight pmd-editor-pickup-selected';
        box.style.top = `${top}px`;
        box.style.height = `${Math.max(2, bottom - top)}px`;
        this.host.appendChild(box);
        this.selectedBoxes.push(box);
      } catch {
        /* skip stale coords during editor updates */
      }
    }
  }

  private removeSelectionHighlights(): void {
    for (const box of this.selectedBoxes) box.remove();
    this.selectedBoxes = [];
  }
  ...
  private createPickupPill(items: DragItem[]): void {
    ...
    const text = items.length > 1 ? `${items.length} items` : label;
    pill.textContent = text.length > 40 ? text.slice(0, 38) + '…' : text;
