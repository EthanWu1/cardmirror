...
interface EvidenceIndexRequest {
  ...
  /** Per-file row allowance; must mirror evidence-index-worker.ts. */
  limits?: EvidenceExtractLimits;
}
...
/** The per-file allowance, sized so the WHOLE corpus fits the global budget.
 *
 *  This is the fix for "it only searches ~600 of my 1700 files". The budget is
 *  a hard bound on rows held in memory (a row costs roughly 0.7-2 KB), and the
 *  indexer stops walking once it is spent — so if files are allowed to consume
 *  it unevenly, the corpus is truncated partway and every later file is
 *  silently unsearchable. Dividing it up front means every file gets indexed;
 *  a small library of big files still gets a deep allowance each, and a large
 *  library trades depth for covering all of it.
 *
 *  `maxRows` counts EVERY kind. Capping only body rows left tags and cites
 *  unbounded, which is how one 400-card file could emit 800 rows regardless. */
export function perFileBudget(
  fileCount: number,
  maxRows: number,
): { maxRows: number; maxBodyRows: number } {
  const share = Math.floor(maxRows / Math.max(1, fileCount));
  const rows = Math.max(PER_FILE_ROWS_FLOOR, Math.min(PER_FILE_ROWS_CEILING, share));
  // Bodies may take most of a generous allowance but never all of it, so a
  // file's tags and cites always survive.
  return { maxRows: rows, maxBodyRows: Math.max(1, Math.floor(rows * 0.6)) };
}
/** Never index fewer rows than this from a file — below it a file is present
 *  in name only. A corpus large enough to hit this floor will exceed the
 *  global budget, and the indexer's own cap remains the backstop. */
const PER_FILE_ROWS_FLOOR = 60;
/** Never index more than this from one file, so a single pathological
 *  document still cannot dominate the run. */
const PER_FILE_ROWS_CEILING = 20_000;
...
export async function indexEvidenceFiles(opts: EvidenceIndexOptions): Promise<EvidenceIndexResult> {
  ...
  // Sized once per run: every file in this run gets the same fair share.
  const fileBudget = perFileBudget(files.length, maxRows);
    ...
    for (;;) {
      ...
      try {
        ...
        const extracted = worker
          ? await withTimeout(
              indexInWorker(worker, entry, file.bytes, file.format, fileBudget),
              EVIDENCE_INDEX_TIMEOUT_MS,
            )
          : ((await opts.indexFileOnMain!(entry, file.bytes, file.format, fileBudget)) ?? []);
