...
import { createDescriptorBuilder, type AnchorDescriptor } from './learn-anchor.js';
...
export interface FileEntry {
  ...
  dirLower: string;
  /** On-disk bytes, when the lister supplied them. The evidence indexer skips
   *  files past its byte cap without reading them; absent means "unknown, read
   *  it and find out". */
  size?: number;
}
...
// ── Evidence rows ────────────────────────────────────────────────────
//
// Searching INSIDE files — tags, cites, card bodies — not just filenames.
// This is what the parallel evidence indexer (evidence-index.ts) feeds, and
// it reuses the ranking helpers above rather than adding a second scorer.
 ...
 *  enough to relocate it uniquely). See `createDescriptorBuilder` in
 *  learn-anchor.ts. */
...
export const EVIDENCE_MAX_BODY_ROWS_PER_FILE = 80;
...
export interface EvidenceExtractLimits {
  /** Hard ceiling on rows emitted for this file, ALL kinds counted.
   *
   *  Capping only body rows was the real "it only searches ~600 of my 1700
   *  files" bug: tags and cites were unbounded, so one 400-card file emitted
   *  800 structural rows and ate its share of the global budget many times
   *  over. The global cap then halted the walk partway through the corpus,
   *  and every later file was silently unsearchable. */
  maxRows?: number;
  /** Sub-ceiling on long body rows, so bodies cannot crowd out the short,
   *  high-signal tag and cite rows within one file's allowance. */
  maxBodyRows?: number;
}

/** How the per-file allowance is spent when it cannot cover everything.
 *  Tags are what people actually search for, cites next; bodies are long and
 *  numerous, so they fill whatever is left. */
const KIND_PRIORITY: Record<EvidenceRowKind, number> = {
  tag: 0,
  pocket: 1,
  hat: 1,
  block: 1,
...
export function extractEvidenceRows(
  doc: PMNode,
  file: FileEntry,
  /** A bare number is read as `maxBodyRows` for older callers. */
  limits: number | EvidenceExtractLimits = {},
): EvidenceSearchRow[] {
  const opts: EvidenceExtractLimits =
    typeof limits === 'number' ? { maxBodyRows: limits } : limits;
  const maxRows = opts.maxRows ?? Number.POSITIVE_INFINITY;
  const maxBodyRows = opts.maxBodyRows ?? EVIDENCE_MAX_BODY_ROWS_PER_FILE;
  ...
  // Upstream exposes the flatten-once builder rather than a raw Flat; the
  // cap is the same one (see createDescriptorBuilder in learn-anchor.ts).
  const buildAnchor = createDescriptorBuilder(doc, EVIDENCE_ANCHOR_MAX_QUOTE);
  doc.descendants((node, pos) => {
    ...
    if (isBodyEvidenceKind(kind)) {
      if (bodyRowCount >= maxBodyRows) return true;
      bodyRowCount += 1;
    }
    const from = pos + 1;
  ...
  // Over the file's allowance: keep the highest-signal rows, then restore
  // document order so callers still see the file read top to bottom.
  return rows
    .map((row, i) => ({ row, i }))
    .sort((a, b) => KIND_PRIORITY[a.row.kind] - KIND_PRIORITY[b.row.kind] || a.i - b.i)
    .slice(0, maxRows)
    .sort((a, b) => a.i - b.i)
    .map((e) => e.row);
}
