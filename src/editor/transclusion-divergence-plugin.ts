...
export const transclusionDivergencePerfProbe = {
  rebuilds: 0,
};
...
function buildDecoSet(doc: PMNode, diverged: Set<string>): DecorationSet {
  ...
  transclusionDivergencePerfProbe.rebuilds += 1;
  ...
  return new Plugin<DivergenceState>({
    ...
    state: {
      ...
      apply(tr, value, _old, newState) {
        ...
        if (tr.docChanged) {
          return { diverged, decoSet: value.decoSet.map(tr.mapping, newState.doc), refreshSeq };
        }
