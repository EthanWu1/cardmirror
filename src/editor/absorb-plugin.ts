...
const ABSORBABLE_TYPES = new Set(['paragraph', 'cite_paragraph', 'undertag', 'card_body', 'table']);

export const absorbPerfProbe = {
  fullScans: 0,
  topLevelVisited: 0,
};
...
export const absorbPlugin: Plugin = new Plugin({
  appendTransaction(transactions, _oldState, newState) {
    ...
    const regions = findAbsorbRegionsNearRange(newState.doc, range.from, range.to);
...
function findAbsorbRegions(doc: PMNode): AbsorbRegion[] {
  ...
  doc.forEach((child) => {
    absorbPerfProbe.topLevelVisited += 1;
...
function findAbsorbRegionsNearRange(doc: PMNode, from: number, to: number): AbsorbRegion[] {
  if (doc.childCount === 0) return [];
  const window = absorptionScanWindow(doc, from, to);
  if (!window) return [];
  return findAbsorbRegionsBetween(doc, window.from, window.to);
}

function absorptionScanWindow(
  doc: PMNode,
  from: number,
  to: number,
  ...
  let start = $from.depth >= 1 ? $from.before(1) : safeFrom;
  let end = $to.depth >= 1 ? $to.after(1) : safeTo;

  // If the edit landed inside loose doc-level content, walk backward through
  // the loose run to include the absorbing card/unit that may own it.
  while (start > 0) {
    const prev = doc.childBefore(start);
    if (!prev.node) break;
    start = prev.offset;
    if (ABSORBING_TYPES.has(prev.node.type.name)) break;
    if (!ABSORBABLE_TYPES.has(prev.node.type.name)) break;
  }
...
function findAbsorbRegionsBetween(doc: PMNode, from: number, to: number): AbsorbRegion[] {
  const regions: AbsorbRegion[] = [];
  let absorbing: PMNode | null = null;
  let absorbingPos = 0;
  let bodiesPieces: PMNode[] = [];
  ...
  doc.nodesBetween(from, to, (child, pos, parent) => {
    ...
    if (ABSORBING_TYPES.has(t)) {
      ...
      // Outside any local absorption zone.
    } else if (t === 'paragraph') {
      bodiesPieces.push(schema.nodes['card_body']!.create(null, child.content));
      regionEndPos = childEnd;
    } else if (ABSORBABLE_TYPES.has(t)) {
      bodiesPieces.push(child);
      regionEndPos = childEnd;
    } else {
      flush();
