...
function appendAlertContent(
  ...
  body.className = 'pmd-alert-message';
...
export interface ChoicePromptOptions<T extends string> {
  ...
  choices: { value: T; label: string; primary?: boolean; danger?: boolean }[];
...
export function promptForChoice<T extends string>(
  ...
  return new Promise((resolve) => {
    ...
    for (const choice of opts.choices) {
      ...
      btn.className = [
        'pmd-text-prompt-ok',
        isPrimary ? 'pmd-text-prompt-primary' : 'pmd-text-prompt-secondary',
        choice.danger ? 'pmd-text-prompt-danger' : '',
      ]
        .filter(Boolean)
        .join(' ');
...
/** Modal choice dialog in the same route-dialog visual vocabulary as the
 *  unsaved-changes prompt (`confirmCloseUnsaved`): route buttons with
 *  number-key shortcuts. Use for confirmations that should read as part of
 *  that family (the co-editing start / close / end flows). Returns the chosen
...
export function promptForRouteChoice<T extends string>(
  ...
  return new Promise((resolve) => {
    ...
    for (const choice of opts.choices) {
      ...
      btn.className = [
        'pmd-route-btn',
        choice.tone === 'primary' ? 'pmd-route-btn-primary' : '',
        choice.tone === 'danger' ? 'pmd-route-btn-danger' : '',
      ]
        .filter(Boolean)
        .join(' ');
...
export function confirmDialogDetailed(
  ...
  return new Promise((resolve) => {
    ...
    if (opts?.checkboxLabel) {
      ...
      checkboxWrap.className = 'pmd-confirm-checkbox';
      checkbox = document.createElement('input');
      checkbox.type = 'checkbox';
      checkbox.checked = opts.checkboxInitial === true;
      const checkboxText = document.createElement('span');
      checkboxText.textContent = opts.checkboxLabel;
    ...
    const finish = (value: boolean): void => {
      ...
      document.removeEventListener('keydown', onKey);
      restoreFocus();
      resolve({ confirmed: value, checked: checkbox?.checked ?? false });
    };
    const cancelBtn = document.createElement('button');
    cancelBtn.type = 'button';
    cancelBtn.className = 'pmd-route-cancel';
    ...
    okBtn.className = opts?.danger
      ? 'pmd-text-prompt-ok pmd-text-prompt-danger'
      : 'pmd-text-prompt-ok';
    okBtn.textContent = opts?.okLabel ?? 'OK';
...
export async function confirmDialog(
  ...
  return (await confirmDialogDetailed(message, opts)).confirmed;
}
