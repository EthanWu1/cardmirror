...
import { fromDocxFull, parseNative, serializeNativeAsync, NATIVE_FILE_EXTENSION, type SharedDocMetadata } from '../index.js';
...
import {
  showPaneRouteOverlay,
  type PaneRouteSlot,
  type PaneRouteChoice,
} from './pane-route-overlay.js';
...
import {
  ...
  confirmCloseUnsaved,
  chooseCloseSaveLocationForActiveFile,
  clearCloseSaveLocationForActiveFile,
  locationLabelFromHandle,
...
import { clampLiveZoom } from './zoom-gesture.js';
...
async function runJournalForRecord(record: DocRecord): Promise<void> {
  ...
  try {
    ...
    const bytes = await serializeNativeAsync(state.doc, {
      ...
      ...(record.sharedDoc ? { sharedDoc: record.sharedDoc } : {}),
    });
...
function cancelPendingPersistenceForCleanRecord(record: DocRecord): void {
  if (record.autosaveTimer !== null) {
    window.clearTimeout(record.autosaveTimer);
    record.autosaveTimer = null;
  }
  if (record.journalTimer !== null) {
    window.clearTimeout(record.journalTimer);
    record.journalTimer = null;
  }
  void clearJournalForRecord(record);
}

function markRecordClean(record: DocRecord): void {
  record.dirty = false;
  record.cleanBaseline = record.view.state.doc;
}

function markRecordChanged(record: DocRecord, doc: PMNode): boolean {
  record.dirty = isMeaningfullyDirtyFromBaseline(record.cleanBaseline, doc, record.format);
  record.editGen++;
  if (!record.dirty) cancelPendingPersistenceForCleanRecord(record);
  return record.dirty;
}
...
class Slot {
  ...
  /** Footer word count. */
  private wcBtn: HTMLButtonElement;
  ...
  constructor(id: SlotId, shell: MultiPaneShell) {
    ...
    // W button opens the Word Count Selection modal scoped to this
  ...
  /** Detach EVERY record from this slot without destroying any view, and
   *  return them in stack order. Used by the tab drag-to-split reflow, which
   *  releases all slots and re-pushes records into their target slots. The
   *  visible record's DOM is unmounted (its view stays live); background
   *  records were never mounted. The slot ends empty + hidden. Deliberately
   *  does NOT fire the notify/reconcile/layout hooks — the caller drives one
   *  reconcile after the whole reflow so intermediate empty states don't
   *  thrash focus + layout. */
  ...
  /** Detach ONE record from this slot without destroying its view. If it's
   *  the visible record, defers to `releaseVisible` (which re-mounts the next
   *  or empties the slot, firing the usual hooks). A background record is
   *  never mounted, so it's just spliced out. Returns the record, or null if
   *  it isn't in this slot. Used by the cross-pane tab move. */
  ...
  private async closeVisibleInner(opts?: { modeSwitch?: boolean }): Promise<boolean> {
    ...
    // Unsaved-file prompt (save / discard / cancel). Save routes through the
    // normal save flow, which opens a picker for never-saved docs.
    // Returns false to
    // abort the close.
    const runDirtyPrompt = async (): Promise<boolean> => {
      const choice = await confirmCloseUnsaved({
        locationLabel: locationLabelFromHandle(closing.handle),
        ...(isDocRecord(closing) ? { onChooseLocation: chooseCloseSaveLocationForActiveFile } : {}),
      });
      if (choice === 'cancel') {
        clearCloseSaveLocationForActiveFile();
        return false;
      }
      if (choice === 'save') return isFlowRecord(closing) ? closing.save() : runSaveFlow();
      clearCloseSaveLocationForActiveFile();
    ...
    if (autosaveBacked) {
      ...
      if (coedited) {
        const keepDurable =
          isDocRecord(closing) && closing.format === 'cmir' && closing.sharedDoc != null;
        const closed = keepDurable
          ? await collabCloseKeepResumable(closing.uid)
          : await collabEndOrLeaveSession(closing.uid);
  ...
  async promptSaveDirtyForQuit(): Promise<boolean> {
    for (const rec of [...this.stack]) {
      ...
      if (recordAutosaveBackedRoute(rec)) {
        ...
        if (coedited && isDocRecord(rec)) {
          const keepDurable = rec.format === 'cmir' && rec.sharedDoc != null;
          const closed = keepDurable
            ? await collabCloseKeepResumable(rec.uid)
            : await collabEndOrLeaveSession(rec.uid);
          if (!closed) return false;
        }
        continue;
      }
      ...
      if (choice === 'save') {
        if (!(await (isFlowRecord(rec) ? rec.save() : runSaveFlow()))) return false;
  ...
  refreshCopresence(): void {
    ...
    const uid = isDocRecord(visible) ? visible.uid : null;
    const cp = uid ? collabCopresenceFor(uid) : null;
    if (!cp) {
      this.copresenceEl.hidden = true;
      this.copresenceEl.replaceChildren();
      this.copresenceEl.removeAttribute('title');
      delete this.copresenceEl.dataset.collabState;
      return;
    }
    this.copresenceEl.hidden = false;
    this.copresenceEl.dataset.collabState =
      cp.connected && cp.queued === 0 ? 'synced' : 'problem';
    this.copresenceEl.title = cp.connected
      ? cp.queued > 0
        ? 'Connected. Local edits are still being sent to the shared room.'
        : 'Connected. Your edits are caught up with the shared room.'
      : cp.queued > 0
        ? 'Offline. Your edits stay in this file and will send when the relay reconnects.'
        : 'Offline. CardMirror is trying to reconnect this shared room.';
    const dot = document.createElement('span');
    dot.className = 'pmd-collab-live-dot';
    const labelEl = document.createElement('span');
    labelEl.className = 'pmd-pane-copresence-label';
    labelEl.textContent = 'Live';
    this.copresenceEl.replaceChildren(dot, labelEl);
  }
  ...
  refreshWordCount(): void {
    const rec = this.visible;
    if (!rec) {
      this.wcBtn.hidden = false;
      this.wcEl.hidden = false;
      this.wcEl.textContent = '-';
      return;
    }
    if (!isDocRecord(rec)) {
      this.wcBtn.hidden = true;
      this.wcEl.hidden = true;
      this.wcEl.textContent = '';
      return;
    }
    this.wcBtn.hidden = false;
    ...
    // regardless of any selection (the W button covers selection counts
    // on demand).
    ...
    // This is the debounced per-pane chrome refresh (heavy-update tick,
    // mount, focus, settings) — the natural point to keep the tab strip's
    // dirty dots + active state current without a per-keystroke rebuild.
  ...
  async closeRecord(rec: PaneRecord): Promise<void> {
    ...
    if (isDocRecord(rec) && collabCopresenceFor(rec.uid) != null) {
      this.showRecord(rec);
      this.shell.focusSlot(this);
      await this.closeVisible();
      return;
...
class MultiPaneShell {
  ...
  /** Chrome-style tab strip below the ribbon (the `tabbedWorkspace`
   *  setting). Null when the setting is off. Surfaces the slots' stacks
   *  as tabs; see workspace-tabs.ts. */
  private tabStrip: WorkspaceTabStrip | null = null;
  /** Coalesces tab-strip refresh requests into one per frame. */
  private tabsRafId = 0;
  /** Drop-zone highlight shown while a tab is dragged into the canvas
   *  (Phase 3 drag-to-split). Created lazily. */
  private tabDropOverlay: HTMLElement | null = null;
  /** The resolved, non-no-op pane arrangement the current drag would apply,
   *  or null when the pointer isn't over a valid drop target. */
  ...
  constructor() {
    ...
    this.unsubscribeSettings = settings.subscribe((s) => {
      ...
      for (const id of SLOT_IDS) {
        for (const rec of this.slots[id].stack) {
          if (isDocRecord(rec) && rec.readMode) {
      ...
      if (s.markUnreadAfterMarker !== shellLastMarkUnread) {
        ...
        for (const id of SLOT_IDS) {
          for (const rec of this.slots[id].stack) {
            if (isDocRecord(rec)) rec.view.dispatch(rec.view.state.tr.setMeta(MARK_UNREAD_TOGGLE, true));
          }
        }
      }
      ...
      if (numberingSig !== shellLastNumberingSig) {
        ...
        for (const id of SLOT_IDS) {
          for (const rec of this.slots[id].stack) {
            if (isDocRecord(rec)) rec.view.dispatch(rec.view.state.tr.setMeta(NUMBERING_REFRESH, true));
          }
        }
      }
    ...
    setAutosaveStateResolver(() => {
      const rec = this.focusedSlot?.visible;
      return isDocRecord(rec) ? rec.autosaveEnabled || sharedDocAutosaveForced(rec) : false;
    });
    ...
    dragController.subscribe((event) => {
      if (event === 'begin') {
        ...
        if (
          ...
          if (isDocRecord(targetSlot?.visible)) {
    ...
    // it as the setting changes (no reload needed — it's a pure overlay on
    // the slot/record model).
    this.applyTabbedMode();
    settings.subscribe((s) => this.syncTabbedModeFromSettings(s.tabbedWorkspace));
  }

  /** Last `tabbedWorkspace` value applied, so the settings subscription only
   *  reacts to an actual flip (settings.subscribe fires on every change). */
  ...
  refreshLayout(): void {
    ...
    // Structural change (open / close / move / expand) → the tab strip's
    // segments changed.
  ...
  reconcileNavRail(): void {
    ...
    for (const id of SLOT_IDS) {
      const slot = this.slots[id];
      const showByLayout =
        (expanded ? slot === expanded : slot.stack.length > 0) && isDocRecord(slot.visible);
    ...
    const anyDocOpen = SLOT_IDS.some((id) => isDocRecord(this.slots[id].visible));
  ...
  private anyNavHidden(): boolean {
    return SLOT_IDS.some(
      (id) => isDocRecord(this.slots[id].visible) && this.slots[id].navHidden,
    );
  }
  ...
  setAllNavHidden(hidden: boolean): void {
    for (const id of SLOT_IDS) {
      if (isDocRecord(this.slots[id].visible)) this.slots[id].navHidden = hidden;
    }
    this.reconcileNavRail();
  }
   ...
   *  These are the docs `promptSaveAllForQuit` would surface a prompt for; the
   *  close handler uses this to decide whether the window close is otherwise
   *  "silent" (and so needs its own generic confirm). */
  anyUnsavedOrLiveForQuit(): boolean {
    for (const id of SLOT_IDS) {
      for (const rec of this.slots[id].stack) {
        if (rec.dirty) return true;
        if (isDocRecord(rec) && collabCopresenceFor(rec.uid) != null) return true;
      }
    }
    return false;
  }
  ...
  focusSlot(slot: Slot): void {
    ...
    // Focused pane changed → the tab strip's focused-segment highlight and
    // active-tab per segment may have moved.
    this.scheduleTabsRefresh();
  }
  ...
  zoomFocusedBy(deltaPct: number): void {
    ...
    if (isFlowRecord(rec)) rec.workspace.setZoom(rec.zoomPct);
    else applyZoomToTarget(rec.editorEl, rec.zoomPct);
    refreshZoomStatus();
  }

  /** Set the focused pane's body zoom exactly for smooth pinch/Ctrl-scroll. */
  zoomFocusedTo(pct: number): void {
    const rec = this.focusedSlot?.visible;
    if (!rec) return;
    rec.zoomPct = clampLiveZoom(pct);
    if (isFlowRecord(rec)) rec.workspace.setZoom(rec.zoomPct);
    else applyZoomToTarget(rec.editorEl, rec.zoomPct);
    refreshZoomStatus();
  }
   ...
   *  name rather than the whole-window title. */
  filenameForUid(uid: string): string | null {
    return this.recordForUid(uid)?.filename ?? null;
  }

  private recordForUid(uid: string): DocRecord | null {
    for (const id of SLOT_IDS) {
      for (const rec of this.slots[id].stack) {
        if (isDocRecord(rec) && rec.uid === uid) return rec;
      }
    }
    return null;
  }
  ...
  setFocusedFile(file: { filename: string; handle: unknown | null; format: DocFormat | null }): void {
    ...
    rec.autosaveEnabled = isAutosaveOnForPath(file.handle, file.format === 'cmir');
    slot.refreshChipFilename();
    pushPaneDocInfo(rec.uid, rec.filename);
  }
  ...
  async openFileIntoSlot(target: SlotId): Promise<void> {
    ...
    if (isMultiPaneFlowRouteFile(opened.name)) {
      await this.loadOpenedFlowIntoSlot(opened, target);
      return;
    }
  ...
  async onFileOpen(opened: OpenedFile): Promise<void> {
    if (await this.surfaceDuplicateIfOpen(opened)) return;
    // Tabbed workspace: opening a file adds a tab to the current pane, no
    // "which pane?" prompt (Chrome behaviour). Splitting into a second pane
    // is a deliberate drag gesture, not a per-open question.
    if (this.tabbedActive()) {
      await this.loadOpenedIntoSlot(opened, this.defaultTabSlot());
      return;
    }
    const choice = await this.promptForOpenRoute(opened.name);
    if (!choice) return;
    if (choice === 'separate') {
      await this.spawnOpenedDocSeparateOrFallback(opened);
      return;
    }
  ...
  private async surfaceDuplicateIfOpen(opened: OpenedFile): Promise<boolean> {
    const existing = await this.findOpenPaneByHandle(opened.handle ?? null);
  ...
  private async loadOpenedIntoSlot(opened: OpenedFile, target: SlotId): Promise<void> {
    ...
    if (!isDocxBytes) {
      ({ doc, threads, docId, sharedDoc } = parseNative(opened.bytes));
    } else {
    ...
    slot.push(record);
    this.connectSharedDocForRecord(record);
  }

  private connectSharedDocForRecord(record: DocRecord): void {
    const sharedDoc = record.sharedDoc;
    void import('./collab/collab-ui.js')
      .then(async (m) => {
        const deps = {
          getView: () => record.view,
          getOwnerUid: () => record.uid,
          getViewForUid: (uid: string) => this.recordForUid(uid)?.view ?? null,
          refreshPlugins: () => {
            record.view.updateState(
              record.view.state.reconfigure({ plugins: buildEditorPlugins(record.uid) }),
            );
          },
          refreshPluginsForUid: (uid: string) => {
            const rec = this.recordForUid(uid);
            if (rec) {
              rec.view.updateState(
                rec.view.state.reconfigure({ plugins: buildEditorPlugins(uid) }),
              );
            }
          },
          setSharedDocForUid: (uid: string, value: SharedDocMetadata | null) =>
            this.setSharedDocForUid(uid, value),
          persistSharedDocForUid: (uid: string, value: SharedDocMetadata) =>
            this.persistSharedDocForUid(uid, value),
          markDirtyForUid: (uid: string) => this.markDirtyForUid(uid),
          newSessionDoc: () => false,
        };
        try {
          if (sharedDoc) return await m.connectSharedDocFlow(deps, sharedDoc);
          if (record.format !== 'cmir') return false;
  ...
  async createNewDoc(): Promise<void> {
    // Tabbed workspace: New adds a tab to the current pane (no slot prompt).
    const target = this.tabbedActive() ? this.defaultTabSlot() : await this.promptForSlot('Untitled');
    if (!target) return;
  ...
  private promptForOpenRoute(filename: string): Promise<PaneRouteChoice<SlotId> | null> {
    return showPaneRouteOverlay({
      filename,
      slots: this.routeSlots(),
      activeSlotId: this.focusedSlot?.id ?? null,
  ...
  setSharedDocForUid(uid: string, sharedDoc: SharedDocMetadata | null): void {
    for (const id of SLOT_IDS) {
      for (const rec of this.slots[id].stack) {
        if (!isDocRecord(rec)) continue;
        if (rec.uid !== uid) continue;
        rec.sharedDoc = sharedDoc;
        return;
      }
    }
  }

  async persistSharedDocForUid(uid: string, sharedDoc: SharedDocMetadata): Promise<void> {
    const rec = this.recordForUid(uid);
    if (!rec) return;
    rec.sharedDoc = sharedDoc;
    ...
    const state = rec.view.state;
    const threads = Array.from(getCommentsState(state).threads.values());
    const bytes = await serializeNativeAsync(state.doc, {
      ...(threads.length ? { threads } : {}),
      ...(rec.docId ? { docId: rec.docId } : {}),
      sharedDoc,
    });
    await getHost().saveExisting(rec.handle, bytes, { force: true });
    commitClean();
    reportAutosaveSuccess();
  }
  ...
  markDirtyForUid(uid: string): void {
    ...
    rec.dirty = true;
    rec.editGen++;
    scheduleAutosaveForRecord(rec);
    scheduleJournalForRecord(rec);
  }
  ...
  refreshSpeechChips(): void {
    ...
    for (const id of SLOT_IDS) {
      const slot = this.slots[id];
      const visible = slot.visible;
      const isSpeech = !!speechView && isDocRecord(visible) && visible.view === speechView;
  ...
  const workspace = createFlowWorkspace({
    ...
    onConfirmDeleteFlow: async (title) => {
      const choice = await promptForChoice<'delete'>({
        ...
        detail: title,
        choices: [{ label: 'Delete', value: 'delete', primary: true, danger: true }],
      });
...
function buildDocRecord(
  ...
  const view: EditorView = new EditorView(editorEl, {
    ...
    dispatchTransaction(tx) {
      ...
      if (tx.docChanged && !isBenchmarkActive()) {
        const dirty = markRecordChanged(record, next.doc);
        if (dirty) {
  ...
  const record: DocRecord = {
    ...
    // Autosave is per-pane in multi-doc. Saved .cmir files default ON;
    // .docx stays manual-save unless the user explicitly enabled autosave
    // for that path.
    autosaveEnabled: isAutosaveOnForPath(opts.handle, opts.format === 'cmir'),
    autosaveTimer: null,
    docId: opts.docId ?? null,
    sharedDoc: opts.sharedDoc ?? null,
...
export function mountMultiPaneShell(): void {
  ...
  enableMultiDocMode({
    ...
    onCreateFlow: () => shell!.createNewFlow(settings.get('defaultFlowFormat')),
    onFlowOpen: (opened) => shell!.routeOpenedFlowFile(opened),
    saveFocusedFlow: () => shell!.saveFocusedFlow(),
    saveFocusedFlowAs: () => shell!.saveFocusedFlowAs(),
    addFlowToFocused: (side) => shell!.addFlowToFocused(side),
    ...
    zoomFocusedTo: (pct) => shell!.zoomFocusedTo(pct),
    ...
    setSharedDocForUid: (uid, sharedDoc) => shell!.setSharedDocForUid(uid, sharedDoc),
    persistSharedDocForUid: (uid, sharedDoc) => shell!.persistSharedDocForUid(uid, sharedDoc),
