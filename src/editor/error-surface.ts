...
/** Whether a save failed because the existing file is still held open
 *  by another process or sync provider. Save As is not the right
 *  automatic remedy here: if the file did not change on disk, the user
 *  usually wants to close Word / wait for Dropbox and retry the same
 *  path, not fork the document. */
export function isFileBusyError(err: unknown): boolean {
  if (typeof err !== 'object' || err === null) return false;
  const { message } = err as { message?: unknown };
  if (typeof message !== 'string') return false;
  return message.includes('EBUSY') || /resource busy or locked/i.test(message);
}
