...
const GESTURE_ZOOM_DEFAULT_ON_MIGRATION_KEY = 'pmd-gesture-zoom-default-on-migrated-v1';
...
export interface Settings {
  ...
  /** Default preset for new native CardMirror Flow documents. */
  defaultFlowFormat: 'ld' | 'pf' | 'policy';
  /** Native Flow AFF sheet background color. */
  flowAffColor: string;
  /** Native Flow NEG sheet background color. */
  flowNegColor: string;
  /** Native Flow range/cell selection color. */
  flowSelectionColor: string;
  /** Default zoom multiplier for new native Flow workspaces. */
  ...
  /** Zoom the editor with a trackpad pinch or Ctrl+mouse-wheel. On by
   ...
   *  governs both. Adjusts the live body zoom smoothly and suppresses
   *  Chromium's own native page-zoom on the gesture. */
  gestureZoom: boolean;
  /**
  ...
  /** When true, toggling between three-pane workspace and one-document-per-
   *  window mode asks for confirmation before reloading and moving documents. */
  multiDocConfirmModeSwitch: boolean;
...
export interface SettingMeta {
  ...
  min?: number;
  /** Ceiling for `kind: 'number'` editors — the input's `max` and the
   *  clamp applied on change. Optional because most legacy number rows only
   *  need a lower bound. */
  max?: number;
  /** Step for `kind: 'number'` editors. Defaults to 1. */
  step?: number;
  /** Options for generic string `kind: 'select'` rows. */
...
export const SETTING_METADATA: SettingMeta[] = [
  ...
  {
    key: 'multiDocConfirmModeSwitch',
    label: 'Confirm layout switches',
    description:
      'Ask before switching between the three-pane workspace and one-document-per-window mode. The switch reloads the editor and reopens documents in the new layout.',
    kind: 'toggle',
    category: 'general',
    section: 'Workspace',
    aliases: ['do not ask again', 'mode switch confirm', 'layout switch confirm'],
  },
  {
    key: 'mobileLayout',
  ...
  },
  {
    ...
    description:
      'Zoom the document smoothly with a trackpad pinch or Ctrl + mouse-wheel. The zoom buttons and Ctrl-= / Ctrl-- chords still move in 10% steps.',
    kind: 'toggle',
    category: 'general',
    section: 'Editor behavior',
  ...
  },
  {
    ...
    description:
      'Preset used when Create Flow opens a new native CardMirror Flow document.',
    kind: 'select',
    category: 'flow',
    section: 'Native Flow',
    options: [
    ...
    aliases: ['flow format', 'flow preset', 'ld', 'pf', 'policy'],
  },
  {
    key: 'flowZoomDefault',
    label: 'Default Flow zoom',
    description:
      'Zoom multiplier for newly-created native Flow workspaces. 1 is 100%.',
    kind: 'number',
    category: 'flow',
    section: 'Native Flow',
    min: 0.5,
    max: 5,
    step: 0.1,
    aliases: ['flow scale', 'flow zoom'],
  },
  {
    key: 'flowAffColor',
    label: 'AFF color',
    description: 'Background color for AFF sheets in native CardMirror Flow.',
    kind: 'color',
    category: 'flow',
    section: 'Native Flow',
    aliases: ['flow aff background'],
  },
  {
    key: 'flowNegColor',
    label: 'NEG color',
    description: 'Background color for NEG sheets in native CardMirror Flow.',
    kind: 'color',
    category: 'flow',
    section: 'Native Flow',
    aliases: ['flow neg background'],
  },
  {
    key: 'flowSelectionColor',
    label: 'Selection color',
    description: 'Highlight color for selected cells and ranges in native CardMirror Flow.',
    kind: 'color',
    category: 'flow',
    section: 'Native Flow',
    aliases: ['flow selection highlight'],
  },
  {
    key: 'flowHostOnLaunch',
    label: 'Legacy Excel Flow bridge warm host',
    description:
      'Windows-only legacy Verbatim/Excel bridge: start the background Excel connection when CardMirror launches so the first old Send to Flow command is faster. Native CardMirror Flow does not use this bridge.',
    kind: 'toggle',
    category: 'flow',
    section: 'Legacy Excel bridge',
    windowsOnly: true,
    aliases: ['flow', 'verbatim flow', 'powershell', 'warm', 'prewarm', 'excel', 'legacy flow'],
  },
  {
    ...
    try {
      ...
      if (raw) {
        ...
        if (parsed && typeof parsed === 'object') {
          ...
          // Builds before smooth pinch/Ctrl-scroll shipped could persist the
          // old default `false` even when the user never chose it. Flip that
          // legacy value once; after this marker exists, an explicit off choice
          // remains respected.
          if (
            parsedRecord['gestureZoom'] === false &&
            localStorage.getItem(GESTURE_ZOOM_DEFAULT_ON_MIGRATION_KEY) !== '1'
          ) {
            parsedRecord['gestureZoom'] = true;
            localStorage.setItem(GESTURE_ZOOM_DEFAULT_ON_MIGRATION_KEY, '1');
            localStorage.setItem(STORAGE_KEY, JSON.stringify(parsedRecord));
          }
          return sanitize({ ...DEFAULTS, ...parsedRecord });
        }
      }
...
function sanitize(s: Settings): Settings {
  return {
    ...
    defaultFlowFormat:
      s.defaultFlowFormat === 'pf' || s.defaultFlowFormat === 'policy'
        ? s.defaultFlowFormat
        : 'ld',
    flowAffColor: sanitizeColor(s.flowAffColor, DEFAULTS.flowAffColor),
    flowNegColor: sanitizeColor(s.flowNegColor, DEFAULTS.flowNegColor),
    flowSelectionColor: sanitizeColor(s.flowSelectionColor, DEFAULTS.flowSelectionColor),
    flowZoomDefault:
      typeof s.flowZoomDefault === 'number' && Number.isFinite(s.flowZoomDefault)
        ? clamp(s.flowZoomDefault, 0.5, 5)
        : DEFAULTS.flowZoomDefault,
    ...
    gestureZoom: s.gestureZoom === false ? false : true,
    ...
    multiDocConfirmModeSwitch: s.multiDocConfirmModeSwitch === false ? false : true,
