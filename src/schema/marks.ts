...
const HIGHLIGHT_NAME_BY_CSS_NAME: Record<string, string> = {
  yellow: 'yellow',
  green: 'green',
  lime: 'green',
  cyan: 'cyan',
...
const HIGHLIGHT_NAME_BY_RGB: Record<string, string> = {
  FFFF00: 'yellow',
  '00FF00': 'green',
  '00FFFF': 'cyan',
  FF00FF: 'magenta',
...
function cleanCssValue(value: string): string {
  return String(value).trim().replace(/\s*!important\s*$/i, '').trim();
}

function parseCssChannel(raw: string): number | null {
...
function normalizeCssColor(value: string): string | null {
  const cleaned = cleanCssValue(value).toLowerCase();
  if (
    !cleaned ||
    cleaned === 'transparent' ||
    cleaned === 'inherit' ||
  ...
  const token =
    cleaned.match(/#[0-9a-f]{3,8}\b/)?.[0] ??
    cleaned.match(/rgba?\([^)]+\)/)?.[0] ??
    cleaned.match(/\b[a-z]+\b/)?.[0] ??
    cleaned;
  ...
  if (hex) {
    ...
    if (raw.length === 3 || raw.length === 4) {
      const alpha = raw.length === 4 ? raw[3] : 'f';
      if (alpha === '0') return null;
      return `${raw[0]}${raw[0]}${raw[1]}${raw[1]}${raw[2]}${raw[2]}`.toUpperCase();
    }
    if (raw.length === 8 && raw.slice(6, 8).toUpperCase() === '00') return null;
  ...
  return CSS_NAMED_COLORS[token] ?? null;
}

function parseFontSizeHalfPoints(value: string): number | false {
  const cleaned = cleanCssValue(value).toLowerCase();
  const match = cleaned.match(/^(-?\d+(?:\.\d+)?)\s*(pt|px)$/);
  if (!match) return false;
  const numeric = Number.parseFloat(match[1]!);
  if (!Number.isFinite(numeric) || numeric <= 0) return false;
  const points = match[2] === 'px' ? numeric * 0.75 : numeric;
  const halfPoints = Math.round(points * 2);
  return halfPoints > 0 ? halfPoints : false;
}

function parseFontFamilyName(value: string): string | false {
  const first = cleanCssValue(value).split(',')[0]?.trim() ?? '';
  const name = first.replace(/^['"]|['"]$/g, '').trim();
  if (!name || /^(serif|sans-serif|monospace|cursive|fantasy|system-ui)$/i.test(name)) {
    return false;
  }
  return name;
}

function parseHighlightColorName(value: string): string | null {
  const cleaned = cleanCssValue(value).toLowerCase();
  const wordName = cleaned.match(/\b[a-z]+\b/)?.[0];
  if (wordName && HIGHLIGHT_NAME_BY_CSS_NAME[wordName]) {
    return HIGHLIGHT_NAME_BY_CSS_NAME[wordName];
  }
  const rgb = normalizeCssColor(value);
  return rgb ? HIGHLIGHT_NAME_BY_RGB[rgb] ?? null : null;
}
  ...
  font_size: {
    ...
    parseDOM: [
      ...
      },
      {
        style: 'font-size',
        getAttrs: (value) => {
          const halfPoints = parseFontSizeHalfPoints(String(value));
          return halfPoints ? { halfPoints } : false;
        },
      },
    ],
    toDOM: (mark) => {
  ...
  underline_direct: {
    ...
    parseDOM: [
      { tag: 'u' },
      {
        ...
        getAttrs: (value) => /\bunderline\b/i.test(String(value)) && null,
      },
      {
  ...
  shading: {
    ...
    parseDOM: [
      ...
      },
      {
        ...
        getAttrs: (value) => {
          if (parseHighlightColorName(String(value))) return false;
          const color = normalizeCssColor(String(value));
  ...
  highlight: {
    ...
    parseDOM: [
      ...
      },
      {
        style: 'background-color',
        getAttrs: (value) => {
          const color = parseHighlightColorName(String(value));
          return color ? { color } : false;
        },
      },
      {
  ...
  font_family: {
    ...
    parseDOM: [
      ...
      },
      {
        style: 'font-family',
        getAttrs: (value) => {
          const name = parseFontFamilyName(String(value));
